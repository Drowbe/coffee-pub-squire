...
| Move the remaining TEN socketlib ops to `gmRequest` and drop the dependency. All are GM-directed chat-card writes -- `createTransfer*Chat`, `createAmmoRequestChat`, `createCompendiumRequestChat`, `retireCardMessage`, `deleteSenderWaitingMessage` -- except the cleanup approval pair, where `cleanupRequestResolved` pushes GM to player and has no `gmRequest` equivalent. That one should not be ported but RESTRUCTURED the way the transfer accept was: `gmRequest.request()` returns the GM's answer to the asker, so the push disappears. Mechanical and low-risk next to the transfer work, but it should wait until the transfer rewrite has been played | High | M | Open |
