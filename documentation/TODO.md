...
| Applying a build should be able to ask the GM first -- swapping gear costs an action and swapping spells needs a rest, so a player re-kitting mid-combat is a table decision. Needs the socket handler, a GM-side apply, and an ask. **The ask is a Blacksmith dialog, NOT a chat card**: chat is noisy and an approval card is easy to miss, which for a permission gate is the failure that matters. A dialog is what the API is FOR -- it is the surface built to carry a choice, it cannot be missed, and it holds enough room to show what is being approved. Not a toast either, with or without buttons: a toast that has to be answered stops being a toast and becomes a floating chat card, and the one-click rule is what keeps toasts safe to fire liberally. Deliberately not half-built: a permission check that looks like a control and is not one is worse than none | High | L | Open |
