...
| `executeItemTransfer` socket takes actor/item ids from any client and moves items as GM without checking the caller is entitled. **The fix is now known and proven**: `blacksmith.gmRequest` hands the handler a caller identity Foundry supplied and the client could not choose, which is exactly the missing piece -- the current socket has to trust a user id out of its own payload, and that is the hole. Re-resolve every uuid in the handler and check ownership against the verified user, as `manager-build-approval.js` does. Doing this also retires the hand-rolled socketlib registration for that op | High | M | Open |
