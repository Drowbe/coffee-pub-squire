...
| Investigate a FULL-BODY image as a third character image, alongside portrait and token, for use beyond this module. Builds already has the idea — `main`, the picture at the centre of the doll — but it lives in a Squire flag on a build, so nothing else can read it and it is per-build rather than per-character. The generalisation is a character-level standing-figure image that any module could show: a party roster, a chat card, a sheet header. Questions to settle before any code: does dnd5e or Foundry already have a slot for it (there is no third `img` on the Actor); should it be a Blacksmith-owned convention so every coffee-pub module agrees on the flag path, which is the same reasoning that put classification in Blacksmith; and does the builder's `main` then become a per-build override of it rather than its own thing | Medium | M | Open |
