...
| GM-mediated item transfers moved off the unauthenticated socketlib op onto `blacksmith.gmRequest`, which hands the handler a caller identity Foundry supplied and the client could not choose. Four ops in `manager-transfer-request.js` -- give, offer, accept, withdraw -- and every uuid re-resolved against the verified user. **Two authorization rules, not one**, because two different people start a transfer: GIVING requires owning the source, and ACCEPTING cannot, since the receiver owns the target and not the source. So an offer is RECORDED on the source actor when the request is made -- the one place in this exchange a receiver cannot write -- and the accept handler validates against it and takes the quantity FROM it rather than from the payload, so accepting an offer of one arrow cannot move the quiver. The chat message and a target-actor flag were both weighed as records and rejected: players may create chat messages with arbitrary flags, and the receiver owns the target. Deleted with it: `handleTransferRequest`, `processTransferResponse` and `setTransferRequestFlag`, all dead, and with them the module's only player-directed socket call | **Critical** | M | Done (unreleased) |
| The remaining TEN socketlib ops moved to `gmRequest`, and **socketlib is no longer a dependency** -- gone from `module.json`, from the README and from the setup guide. Six card writers collapsed into ONE op with a whitelisted dispatch (`manager-gm-cards.js`), because they were the same op differing only in which builder they called; `retireCard` and `deleteWaitingCard` keep their own, since they take a message rather than a card. The cleanup approval was RESTRUCTURED rather than ported: `cleanupRequestResolved` pushed GM-to-player and `gmRequest` has no player-directed form by design, but it only existed because the request was fire-and-forget -- `request()` returns the handler's value, so the approval window now resolves a promise the asker is already awaiting and the push is gone. Deleted with it: the whole `socketlib.ready` block, `socket.close()`, and eight now-unused imports in `squire.js` | High | M | Done (unreleased) |
| Migrate chat cards to the Blacksmith Chat Cards API | High | L | Done (13.11.0) |
| `tools/check.mjs` catches CALLED-BUT-NEVER-IMPORTED. A free identifier is valid syntax, so `node --check` passes it and it only fails when the line RUNS -- which for a chat-card button means in front of a player. It shipped twice in one day (`socket is not defined`, then `deleteWaitingCard is not defined`), both times a call site rewritten to use another module's helper with no import added. Not scope analysis, which would want a parser: it asks one narrow question -- if a file calls a bare `name(...)` that another module in `scripts/` exports, does this file import or define it? Comments and string literals are stripped first, and method and accessor definitions counted as declarations; without both, the first run reported six false positives beside the one real bug. **Still not a linter**: it sees nothing about a name no module exports, and nothing about scope WITHIN a file -- the kind that put a `const socket` in a sibling block | Medium | S | Done (unreleased) |
| One `...` menu on every list row, replacing the icon cluster. Weapons, Spells, Inventory, Features and Favourites show the same entries in the same order from `manager-item-menu.js`; the item's NAME opens the sheet, which is what the feather did. The test for what stays on the row is **how often** a thing is done rather than how important it is — a toggle you flip every fight is worth a click, an action you take twice an evening is not — so favourite, equip and prepared kept their glyphs and the feather and send-to went into the menu. `buildItemMenu` is split from `showItemMenu` so a panel can APPEND to the shared set rather than replace it, which is how Favourites adds reorder and tile size without the row meaning something different there | Medium | M | Done (unreleased) |
| The tray's tile layout is one implementation, not two. `utility-tile-spans.js` owns the four footprints, their icons and the Tile Size menu; `styles/tray-tiles.css` owns the grid under a single `squire-tile-grid` class. Both were duplicated between the Favourites and Builds panels — the CSS as eighteen rules in `panel-favorites.css` each carrying a second `.builds-panel.layout-tiles` selector, so one panel's tile layout was defined inside another panel's stylesheet. A panel that wants tiles now adds the class; a panel that wants to differ overrides from its own file | Medium | S | Done (unreleased) |
| Confirm the cleanup preview against the next tagged Blacksmith. Audited: the only resolver call is `resolveMany` in `utility-cleanup.js`, reached from `CleanupWindow`, which a player can open on their own sheet. Once Blacksmith filters pack visibility inside the API a player's preview will offer fewer link candidates than a GM's — acceptable, because the write is never the player's: `openCleanupApproval` re-scans on the GM's client and `_narrowToRequest` filters that authoritative scan to the requested ids. Nothing to change unless the narrowed preview reads as broken in play | Medium | S | Open |
| Delete `CompendiumSearchUtility._isVisibleToUser()` once running a Blacksmith that filters visibility upstream | Medium | S | Done (13.11.0) -- moot: the whole file went with the tray's compendium search |
| Applying a build asks the GM first, when the table wants it. Two world-scoped settings (**GM Approves Build Changes**, on; **GM Approves Costume Changes**, off), a Blacksmith `dialog.wait()` on the GM's screen with Approve/Deny as peers, toasts for the player. Built on `blacksmith.gmRequest`, which supplies a caller identity the requester could not forge -- the handler re-resolves the actor and checks that the VERIFIED user owns it. Dismissing the dialog denies; a Blacksmith too old to have `gmRequest` refuses rather than allows. Not a permission system and labelled as such in the code: a player can still equip from their own sheet, and what this stops is one-click re-kitting mid-combat. **This is the pilot for the rule the whole module now follows: a dialog decides, a toast informs, a chat card records** | High | L | Done (13.11.0) |
| Learn from import corrections. Every dropdown a player changes in the import window is evidence that the automatic answer was wrong, and it is currently thrown away. Two uses, not one: an immediate per-world override keyed on `_stats.compendiumSource` (falling back to a normalised name for homebrew), and an aggregate handed to Blacksmith as the input for a better GENERAL rule. Only learn from an explicit change -- accepting a default is not evidence, the player may not have looked -- because the failure mode of learning is confidently wrong and sticky, where one misclick becomes permanent policy. **Split along the existing boundary**: LOCATION (back, neck, waist) is classification and belongs to Blacksmith's `equipLocations`, so corrections go to them rather than into a private table here, or Squire rebuilds the thing it deliberately gave away; SLOT within a location (sheath vs off-hand, ring1 vs ring2) is occupancy and is ours. That means the half Squire can build alone is the smaller half, so the location half needs raising with Blacksmith first. Tension to hold onto: a per-item learned table is a specific-items table, which is what "general rules, not specific items" ruled out -- the defensible difference is that written rules are guesses and a correction is ground truth, which only holds if the corrections feed a better rule rather than replacing one | Medium | L | Open |
| A FULL-BODY image is a third character image, alongside portrait and token: a plain path in the `fullbody` module flag on the actor, readable from outside Squire. Written by Update Prototype Token, read by a build's unset `main`, and pulled into a costume by its import button (falling back to the portrait). **Still open, and the reason this row survives**: nothing outside Squire reads it yet, and it is a Squire-namespaced flag rather than a suite-wide convention. If a party roster or a chat card should show it, the flag path wants raising with Blacksmith so every coffee-pub module agrees on where it lives -- the same reasoning that put classification there | Medium | S | Partial |
| **Raise with Blacksmith**: durable gear location stamped onto the item as a flag, with a "update world/compendium items" tool, as an alternative or supplement to a lookup table. Assessment before raising it: as the PRIMARY mechanism a stamp is worse than a derived table, because classification rules improve and stamped data goes stale invisibly — the item keeps last year's answer and nothing can tell that from a deliberate one — and it means bulk-mutating the user's world, which Squire's own read-only principle only allows as explicit GM-invoked repair. Items dragged fresh from a compendium also arrive unstamped, so the table is still needed underneath. Where a flag IS right is the manual override: "the GM has decided THIS item goes here, whatever the engine says", which a rule change must never silently undo. So: derived table as the answer, per-item flag as the exception, and the bulk tool only as a way to apply corrections people have already made | Low | M | Open |
| Move the EXISTING transfer and compendium-request approvals off chat cards onto Blacksmith's dialog API. Both currently whisper a card with Approve/Deny buttons, which works and is easy to miss in a busy chat log -- and a GM who misses one leaves a player blocked with no signal that anything is waiting. The card is not wasted: keep it as the RECORD of what was decided, which is what chat is good at, and move the DECISION to a dialog. **The rule this settles, across the module: a dialog decides, a toast informs, a chat card records.** Each surface does one thing and nothing is asked to be two of them. **The pattern is proven** -- see the build-approval row above, tested with a real player client. Follow `manager-build-approval.js`. Use `dialog.wait()`, which takes arbitrary buttons -- `{ action, label, icon, default, destructive, disabled, callback }` -- rather than `confirm()`, since Approve/Deny is two peers and not a yes-with-a-way-out. Read the pressed button off `value`, NOT `action`: the result's `action` is only ever `submit`/`cancel`/`close`. Two properties of the API that decide the design: every helper RESOLVES on dismissal and none rejects, so a GM who closes the dialog resolves `closeValue` (default `false`) -- which for an approval is the correct fail-safe, an unanswered request is a denied one, and the requester has to be told that rather than left waiting; and `destructive: true` brings modality with it, which is what stops an approval being clicked past. Open question before this is relied on at a busy table: the API documents no queueing or stacking behaviour, so check what happens when a second request arrives while the first dialog is up | Medium | M | Open |
| Let the handle filter which statuses it shows (`showHandleConditions` is all-or-nothing) | Low | M | Open |
| Unified tray filter bar | High | L | Done (13.11.0) -- superseded by section tabs; 14 chips are now 5 tabs, 5 action chips and 2 toggles, with favourites in a view of its own |
...
- [x] **`executeItemTransfer` is an unauthenticated socket.** DONE (unreleased). Replaced by four `gmRequest` ops in `manager-transfer-request.js`, which get a verified caller identity from the envelope and re-resolve every uuid against it. The answer to "what should authorization look like" turned out to be two rules, not one: **giving** needs ownership of the source, and **accepting** cannot, because the receiver owns the target and not the source. So an offer is recorded on the SOURCE actor when the request is made — the one place in the exchange a receiver cannot write — and the accept handler validates against it and takes the quantity from it. The chat message and a target-actor flag were both considered and rejected as records: players can create chat messages with arbitrary flags, and the receiver owns the target.
