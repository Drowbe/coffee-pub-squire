...
- [x] **Character and User picker migrations (13.3.19)**: Migrated the final two legacy `Application` (V1) windows to `BlacksmithWindowBaseV2`:
  - `window-characters.js` (`CharactersWindow`)
  - `window-users.js` (`UsersWindow`)
  - Inventory/weapon transfer and private-note recipient callbacks are preserved. Blacksmith now owns the frame, sizing, minimization, viewport handling, and position persistence; the V1 wrappers, listener workarounds, manual positioning settings, and frame CSS were removed.
...
- [ ] **REFACTOR** The *v13*-deprecated APIs still run in v14 with console warnings but are removed in v15/16. The remaining work is the V1 `Dialog`/`Dialog.confirm` call sites (heaviest: panel-quest, panel-codex, panel-notes, and utility-journal); migrate them to `foundry.applications.api.DialogV2`. The custom V1 `Application` windows are now fully migrated. (`ImagePopout` ✓ completed July 14, 2026; Status Effects ✓ 13.3.17; Dice Tray, Macros, and Health ✓ 13.3.18; Character and User pickers ✓ 13.3.19.)
  ...
  | `scripts/manager-panel.js:997` | `new Dialog` | Select item quantity before a tray transfer | Shared transfer workflow | Yes—unified | Fold into one shared Blacksmith V2 **Transfer Item** window. Preserve the current quantity selector's polished layout and behavior exactly; migration must not make this interaction worse. |
  ...
  | `scripts/panel-inventory.js:519` | `new Dialog` | Select inventory transfer quantity | Shared transfer workflow | Yes—unified | Use the shared Blacksmith V2 **Transfer Item** window and preserve the existing high-quality quantity controls. |
  ...
  | `scripts/panel-party.js:642` | `new Dialog` | Select item quantity for a party-panel transfer | Shared transfer workflow | Yes—unified | Use the same Blacksmith V2 **Transfer Item** window; do not maintain a party-specific copy. |
  ...
  | `scripts/panel-weapons.js:541` | `new Dialog` | Select weapon transfer quantity | Shared transfer workflow | Yes—unified | Use the shared Blacksmith V2 **Transfer Item** window and preserve the current quantity-selector design. |
  | `scripts/squire.js:2370` | `new Dialog` | Recipient approves or rejects an item-transfer request | Shared transfer workflow | Yes—unified | Make this the recipient/approval state of the same **Transfer Item** window so sender quantity and recipient approval share one visual and data contract. |
  ...
  **Blacksmith proposal:** See `documents/proposal-blacksmith-transfer-dialog-api.md` for the proposed reusable Transfer/Share workflow, DialogV2 helpers, API contracts, ownership boundaries, examples, phases, and acceptance criteria.

  **Migration order:** first build the shared Transfer Item window without changing the current quantity-selector experience; then convert the simple confirmations; consolidate Notes/Quest pin maintenance only where it produces a genuinely better tool; build the shared Journal Picker and data-export window; finally remove the duplicate Quest import/export paths and move all JSON imports to Blacksmith's robust importer.
