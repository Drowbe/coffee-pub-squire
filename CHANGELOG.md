...
- **The prepared grid is a fixed 5×5, and the cells past the limit are shown.** Twenty-five is the most any class can ever prepare, and drawing all of them — with the ones beyond this character's limit dimmed and refusing drops — turns the grid into something you can learn the positions of. Sized to the class it was a different shape for every character and lined up with nothing; as a column beside the doll it made a caster's window 320px wider than a fighter's. Five across is the doll's own column count, so it sits under the body in the same rhythm. Square, like every other slot in the window. The dimmed cells are a statement rather than dead space: they are what levelling up will buy.
  - The drop handler refuses a beyond cell as well as the CSS. `pointer-events: none` is a cursor hint, not a permission check.

- **Carried consumables**, for the characters who have no daily preparation. A fighter's doll had a hole where a caster's prepared grid goes, and potions, scrolls, poisons and oils are the closest thing a martial has to one — chosen before the day starts, gone by the end of it, and exactly the thing a plan that names your longsword ought to name too. Ten cells, two rows of five, the same square grid as the spell side. Counted in the build's weight, since they are on the character's back the same as the armour is.
  - It takes **any physical item**, not `type === 'consumable'`. A scroll is often `loot`, a flask of acid is often a thrown `weapon`, and a bag of caltrops is whichever the compendium author felt like — twice already a rule written from `CONFIG` rather than from real content has refused something obviously correct. The grid's name is the guidance; the player decides what counts as spendable.
...
- **Gear builds.** A **Builds** section above Favorites holds a tile per build; the **+** in its header makes one and opens it. A build is a paper doll — sixteen slots around the character's portrait, laid out head/face/neck, back/chest, arms/hands, rings and hips around a waist, feet, then main hand / both hands / off hand — and you drag items from the tray into them. Built on Blacksmith's tool window (`BlacksmithToolWindowBaseV2`), styling content only and taking its colours from the `--blacksmith-tool-*` variables rather than fighting the shell.
  ...
  - **The caster column is two columns of slots, filled downward**, dividing the doll's height so the two halves of the window end level. It was four across and square, which put a wizard's seventeen into five ragged rows lining up with nothing. Twenty-five is the most any class prepares, so a fixed thirteen-row grid would be mostly empty for the cleric who prepares nine — the row count is `ceil(max / 2)`, written inline by the template because only the data knows it.
  ...
  - **The slot readout moved to the head of the column**, once rather than per class. Spell slots are the character's, not any one class's — a multiclass caster casts from a single pool — so a copy inside each class block would be the same numbers claiming to be different ones. At the top it reads as the header for everything below it, which is what it is: the thing that decides whether the lists are castable.
