...
### Changed
- **The handle has a width toggle, above the collapse caret.** Minimal is a single 32px column at 46px wide; Full is a 44px column at 60px — bigger portrait and favorites, and conditions packed two to a row. Same zones in the same order either way; only the sizes change. The choice is per-user and stored in a `handleMode` setting that is deliberately not on the settings page, since the only place the difference is visible is the handle itself.
  - **While the tray is open, the handle is always minimal**, whatever the setting says. Open, the tray is right there, so a wide strip beside it buys nothing and costs the panels 14px. The button still shows lit in that state, because it reports the *setting* — dimming it while the tray is open would make it look off at the moment you were deciding whether to turn it off.
  - This is the earlier two-layout idea, rebuilt on the half that survives. That version changed the *content* on open — controls only, everything else hidden — and died on the pinned tray, since pinned means open permanently and so meant the reduced handle forever. Changing the *width* survives that, because width is a cost that only matters while the panels are competing for the space.
  ...
  - How many favorites fit is re-decided on the handle's `width` transition rather than from the three things that can cause one. The icons are a different height in each mode, so the count changes with the width, and the width is the one signal the mode toggle, the open and the close all share.
