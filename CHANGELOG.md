...
- **Gear builds.** A **Builds** section above Favorites holds a tile per build; the **+** in its header makes one and opens it. A build is a paper doll — sixteen slots around the character's portrait, laid out head/face/neck, back/chest, arms/hands, rings and hips around a waist, feet, then main hand / both hands / off hand — and you drag items from the tray into them. Built on Blacksmith's tool window (`BlacksmithToolWindowBaseV2`), styling content only and taking its colours from the `--blacksmith-tool-*` variables rather than fighting the shell.
  ...
  - **The window has a ground**: `panel-background-03`, a mottled dark green with its own vignette, under a light scrim. A texture rather than a scene, and that distinction is the whole of it — a photograph was tried first and fought everything on top of it, because a daylit field has its own subjects and its own bright and dark regions, and sixteen dark boxes with pale captions land wherever the composition happens to put them. A texture reads as the surface the slots are laid on rather than as a picture they are covering up, which is the only job a background in a working window has. It sits at roughly a fifth visible under a near-opaque scrim, which is the point: what survives is the mottling and the vignette, and at anything stronger it becomes a green thing behind the slots rather than the ground under them. Applied to Squire's own content root rather than the shell, since the Blacksmith window API owns the frame and an id-level selector over it can silence minimisation.
  ...
  - **Slots enforce exactly where dnd5e has a field to answer the question, and nowhere else.** There is no general equipment-slot concept in the system, but "most slots cannot be checked" is not "none can":
    ...
    - **`ring` is a real `equipmentTypes` key**, so the ring slots only take rings.
    - **`ammo` is a real `consumableTypes` key**, so the ammo slots only take ammunition — a healing potion is refused.
    ...
    - Head, Face, Neck, Back, Chest, Arms, Hands, Waist, Hip and Feet take any physical item, because a helm, a cloak, a belt and a pair of boots are all `trinket` or `wondrous` or `clothing` with nothing to tell them apart.
    - **Main Hand and Off Hand take any physical item too.** "Weapon or shield" was written first and refused a torch, which is one of the most ordinary things a hand holds — along with a holy symbol, a wand, a lantern, or a potion about to be drunk. There is no `holdable` field, and inventing one refused more real cases than the imaginary ones it caught.
