...
- **Cleanup's checkboxes are drawn rather than left native.** Foundry themes bare inputs, so an unstyled checkbox changed size and colour with the Foundry theme while the row around it followed the Blacksmith tool theme, and the two never agreed. Selection now also paints the row, driven by `:has(input:checked)` rather than a JS-applied class — the approach Blacksmith's own entity list uses, and for the same reason: a class can drift out of step with the input, a `:has()` cannot.
