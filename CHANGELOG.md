...
- **The pack column**: twenty-six cells, two across and thirteen down, beside the doll. Casters fill it with prepared spells; everybody else fills it with what they carry — potions, scrolls, poisons, oils, the things chosen before the day starts and spent during it. **One column, not two features in the same place**, which is what makes a fighter's window and a wizard's the same size and the same shape.
  ...
  - The window is **600 + rail** rather than 520, because the doll now shares its width with the column. Holding the old figure would have paid for the column by shrinking every gear slot.
  ...
  - The carried half takes **any physical item**, not `type === 'consumable'`. A scroll is often `loot`, a flask of acid is often a thrown `weapon`, and a bag of caltrops is whichever the compendium author felt like — twice already a rule written from `CONFIG` rather than from real content has refused something obviously correct. The column's name is the guidance; the player decides what is worth carrying. Carried items count in the build's weight, since they are on the character's back the same as the armour is.
  - The one place in the window that shows a **quantity**. Everywhere else a slotted item names the kind of thing — one arrow means "arrows" — but a slotted potion means the potions, and how many there are is the whole point of packing them.
