...
- **A costume sets how the token is DRAWN, not just what it is drawn with**: its dimensions in grid spaces, the image fit mode, and the scale — Foundry's own three token-config settings, applied along with the artwork. The picture was only ever half of what a costume changes: a giant in disguise is not the same size on the map, and a portrait cropped tall wants a different fit from one cropped square to sit right in the same box.
  ...
  - **The dropdown's tooltip says what the chosen fit does to the scale**, and changes with it — the slider means something different under each mode, and one whose meaning changes silently is worse than one with no label at all. It was a permanent line of italics under the whole row, which is a lot of standing furniture for a fact about one control.
  ...
  - **Only what CHANGES is marked**, with an amber warning triangle. The first version marked the opposite: a "no change" tag on every untouched control, which put three notices on the commonest state and none on the one worth noticing. The same inversion went through the image slots, which carried a "no change" caption for the same reason and now carry the same triangle when they will actually set something — one glyph meaning one thing across the whole window. The build's own picture lost its caption outright: it is never written to the character, so there was no change to announce either way.
