...
### Changed
- **Dice Tray Blacksmith tool-window migration**: Migrated the standalone Dice Tray from legacy `Application` V1 to Blacksmith's compact `BlacksmithToolWindowBaseV2`. The tray keeps its existing formula builder, advantage/disadvantage rolls, modifiers, and recent-roll controls while gaining the shared parchment shell, native Application V2 dragging, focus, minimize/close behavior, viewport constraints, and automatic per-user position persistence. Registered it with the Blacksmith Window API for menubar, macro, and cross-module opening; removed the V1 listener/frame/position workarounds, redundant dark frame/body styling, and obsolete `diceTrayWindowPosition` setting.
