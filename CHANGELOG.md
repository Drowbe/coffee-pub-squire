...
### Added
- **Status Effects window – Blacksmith Application V2 migration**: Replaced the inline legacy Add Effect dialog with a registered Blacksmith Window API window. The new window uses the shared shell, shows live condition state for the selected actor, preserves GM add/remove behavior, and includes a Remove All action that clears conditions without deleting unrelated ActiveEffects.
