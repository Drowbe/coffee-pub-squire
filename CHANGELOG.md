...
- **An image slot that is not set says so.** It shows the character's *current* artwork when the build has not chosen one, which made it look chosen — and a build writes only the images it actually sets, so an unset portrait changes nothing on apply and looked like it should. The dashed border was carrying that alone and was too quiet.
