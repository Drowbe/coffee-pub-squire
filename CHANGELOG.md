...
- **The prepared column**: twenty-six cells, two across and thirteen down, beside the doll, holding what a caster has prepared. It briefly held **carried potions** for everybody else, and that was cut: everything else in this window is a claim that Apply *enforces*, and there is no carried state in the system to set — so those cells looked exactly like the ones around them and did nothing. A martial's build is the doll. Their window is narrower by this column rather than padded out with something inert.
  ...
  - **The window's height never changes.** The workspace's second row is pinned to the doll's height, so a costume is exactly as tall as a build, both are the same for every character, and throwing the switch cannot resize anything. Toggling used to give a different size than opening straight into costume mode — two paths to the same state that did not agree, which is the kind of thing that means the size is being derived from whatever happens to be on screen. Now there is nothing left for it to depend on.
    - The one place a caster's window and a martial's still differ in costume mode is width, since the window keeps its size and the pictures take the extra rather than leaving a 117px strip of nothing down one side.
