...
- **Build tiles are rows carrying the build's own gear.** Up to five item pictures, the name, and the figures that distinguish one build from another: estimated AC, slots filled, spells prepared, weight, and a warning count when items have left the sheet. They were 104px squares holding a generic shirt and a fraction, which said nothing about the difference between two builds — the only thing a list of builds is for. Every figure comes from one `buildSummary()` pass, so a tile and its window cannot disagree.
