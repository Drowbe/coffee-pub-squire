...
### Added
- **Fill From Sheet shows you the mapping and lets you argue with it.** Every equipped item gets a row and a dropdown of every slot it could legally occupy, with the importer's proposal already selected and *Not equipped* always available. The items nothing could place sort to the **top**, defaulting to Not equipped: those are the decisions still to be made and the rest are already made. Casters planning preparation get the same table for spells — Prepared or Not prepared — with a live count against the limit, since this is the one screen where it can be exceeded.
