...
### Fixed
- **HTML escaping no longer builds a DOM node per call**: `escapeHtml()` used a `createElement`/`textContent`/`innerHTML` round-trip. It is called up to twice per related name and per location level, so a real 314-entry codex was paying thousands of DOM node constructions on **every** render — every pin toggle. Now a regex replace. It also fixes a correctness hole: the DOM approach leaves `"` and `'` unescaped, and the value is interpolated into a `data-uuid="…"` attribute.
