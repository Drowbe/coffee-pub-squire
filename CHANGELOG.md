...
- **Item creation no longer collides with dnd5e's encumbrance effect**: creating an item that pushed an actor over an encumbrance threshold could throw `The _id [dnd5eencumbered0] already exists within the parent collection`. dnd5e recomputes encumbrance after every item create, update, and delete, and that recompute is a check-then-create against one fixed effect id with no lock. Squire wrote twice in a row — create the item, then set the `isNew` flag on it — and the second recompute read the same still-empty effects collection and tried the same id. Both writes were individually awaited and still collided, because the recompute outlives the write that triggered it.
  - The `isNew` flag is now injected at `preCreateItem` via `updateSource`, so it is part of the original write and no follow-up write exists. One create, one recompute. The badge behaves identically; the durable half just arrives with the document.
  - This affects **every** item created on an owned actor, not only transfers — the generic create hook stamped every item this way, so any module or system action that created several items on one actor could produce a rejection per item. Reported by Blacksmith's harness after the transfer-path fix landed, which had only covered Squire's own six creation sites.
  - The follow-up write also made stack-merge identity timing-dependent for anything comparing item flags: a stamped item and a not-yet-stamped one compare as different documents, so identical stacks merged or didn't depending on whether the write had landed. Squire now also calls Blacksmith's `inventory.registerTransientFlag('coffee-pub-squire.isNew')` on ready, declaring the flag as carrying no identity so the merge predicate ignores it for every consumer without any of them needing to know Squire exists.
  - The hook is registered natively rather than through Blacksmith's hook manager: `preCreate` hooks cancel the operation when a handler returns `false`, and routing a cancel-capable hook through a wrapper whose return value Squire doesn't control risks silently blocking item creation world-wide.
