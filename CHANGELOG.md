...
- **The Gear Builds doll is one shape for every character now**, instead of swapping between a martial's and a caster's. Row 6's corners are two more Utility slots (four now, all four accepting anything — spells and features included, which they used to refuse). Below row 6, an always-present **ability row** (Primary/Secondary/Tertiary) and an always-present **weapon row** (Main Hand/Both Hands/Off Hand) replace the single "big three" that used to change meaning by class — a martial who picks up a scroll or a caster who draws a blade now has a slot waiting for it. The window is one row taller to fit. No migration needed: every reused slot key kept its name, so existing builds land in the new layout with nothing missing.
  ...
  - Utility never shows the amber **Not equipped** warning. Every other slot claims its item will be equipped or prepared; Utility just claims a player put something there, so it is not compared against either.
...
- **The import button is on costumes too.** Same button, same glyph, same idea — fill this entry from the character as they are now; only what gets taken differs. A build takes the gear and asks where each piece goes; a costume takes the three pictures and the token's geometry, which needs no questions and so gets a plain confirmation instead of a mapping window.
  - **The geometry comes with it**, which the gear import has no equivalent of: a costume that recorded a face but not the size it was drawn at would put a large character back at 1×1 the moment it was worn.
  - **The full-body image falls back to the portrait**, and the toast says when it did. A character who has never had one set would otherwise get a costume with an empty middle, and a face is a true-if-incomplete answer to "what do they look like" where blank is no answer at all.
...
- **The GM can be asked before a player changes what their character is wearing.** Two world-scoped settings in a new **Gear Builds** section: **GM Approves Build Changes** (on by default) and **GM Approves Costume Changes** (off). Equipping a build changes what a character can do in a fight and unequips everything the build does not name; a costume changes how they look. Different sizes of act, so different defaults.
  ...
  - Registered on **every** client, not only a GM's: any client can become the answering GM, and one that registered nothing answers `UNKNOWN_OP`.
  - **Dismissing the dialog is a denial.** Blacksmith's dialogs resolve on dismissal rather than rejecting, and for an approval that is the right fail-safe — an unanswered request must not become an approved one, and the player is told rather than left waiting on a dialog that is gone.
  - **A GM is never asked**, since they would be asking themselves. If Blacksmith is too old to have `gmRequest`, the change is refused rather than allowed: a gate that lapses silently the moment a dependency is a version behind is worse than one that is honestly unavailable.
  - **World-scoped, both of them.** A user-scoped setting here would be a checkbox the person being asked about gets to untick.
  - **It is not a permission system and does not pretend to be.** A player owns their actor and can equip anything from their own sheet without going near this window. What it stops is re-kitting through Squire in one click mid-combat, which is the thing that actually happens.

- **Favourite a build or costume from its right-click menu**, and it appears on the Favorites tab. An easy way back to the two or three you actually use once the rail is a sea of costumes and one-off kits — which is all it is: it does not equip anything, does not reach the tray handle, and changes nothing about the character. First in the menu, no confirmation, and undone by pressing it again.
  - **Not the same list as the builds pinned to the tray handle**, though both mean "one I reach for". The handle is a strip with room for a few and costs screen space; this is a filter on a list and costs nothing, so a dozen can be starred without consequence.
  - Stored as a field on the build rather than a list of ids in a second flag, so deleting a build takes its heart with it and the two can never disagree about what exists.
  - **A duplicate is not born a favourite.** You copied it to change it; whether the variant earns a heart is a decision to make after seeing it.
  - **The tray's own red heart**, not a star — a favourite is a favourite wherever it appears in this module, and a second glyph would have invented a rival mark for the idea the heart already carries on every item row, in the panel heading and on the mode toggle. It shows on the tile on every tab, not only where it is redundant: on Builds and Costumes it is the only way to see which entries are favourited without switching.
...
- **`Favorites | Costumes | Builds` tabs above the build selector.** Two different kinds of thing share one rail, and past a handful of each, finding the costume you wanted meant reading past every build to get to it. Styled as the tray's section tabs — flat, underlined, an equal share of the width each — because this is the same *kind* of choice the character sheet's All / Weapons / Spells strip makes, and the two are read minutes apart in the same session. Blacksmith's `blacksmith-tab` was tried first and is wrong for the job: it is a chip, and three chips read as three buttons rather than as one control with three positions.
  ...
  - **Each tab offers only the New button that makes something it can show.** All has both; Builds has New Build; Costumes has New Costume. On Builds, a Costume button would make a thing the list in front of you cannot hold — the filter would have to give way for it, and a button that silently moves you somewhere else is a worse offer than no button. The one that remains fills the strip on its own, since the two always shared it equally.
  - The pair on the empty page still offers both, because there is nothing to filter yet — and those two do switch the tab if they have to, so what they made is visible rather than created, selected, put on the doll and absent from an unchanged list.
  - Each tab's count is in its tooltip rather than on its face; the three labels only just fit across the rail as it is.
  - An empty tab says **No costumes yet** rather than showing a blank strip, which looks like a window that failed to load rather than a filter doing its job.
- **Update Prototype Token**, on a costume's right-click menu, writes its portrait and token art onto the character — without wearing it. Wearing a costume changes how they look *now* and repaints every token of theirs standing on the canvas; the prototype token is the stamp for tokens made *later*, so this decides what the next one dropped on a scene looks like and leaves the map alone. That is the difference between dressing a character for one scene and settling what they actually look like, and only the first should follow them into the next scene.
  - **Two images, and nothing else.** The costume's size, fit and scale are not written, even though they live on the prototype token and it would be tidy to carry them along. How big a token is drawn is a different decision from what it is a picture of, and this action asks about the second — an entry called "update prototype token" quietly resizing every future token is a surprise. Wearing the costume is where geometry gets set deliberately.
  - Costume only. A build has no artwork of its own to adopt, and its gear is not something a prototype token knows about.
  - It asks first and names the distinction, because the two actions are easy to confuse and only one of them is undoable from a toast.

- **Set As Default Artwork**, on any entry's right-click menu, makes its three pictures what a **new build or costume starts from**. Every image slot a build leaves unset resolves through the default, so this is the answer they all inherit — make a costume after pressing it and its portrait, token and centre picture are already these, rather than whatever the character happened to look like the first time the builder was opened.
  - **It is not Update Prototype Token, and it sits right beneath it.** That one is about the *character*: it writes artwork Foundry itself uses, on the sheet, in the sidebar and on the next token placed. This one is about *this tool* and touches nothing Foundry can see. Adopting a face for the next scene's token and deciding what your next costume starts from are unrelated intentions, so they are unrelated actions; each dialog says which of the two it is and names the other.
  - **Three pictures, where the prototype token takes two.** The build image at the centre of the doll has no actor field to live in, so the flag is the only place it can be remembered at all — and it is the one a new build most obviously starts from.
  - Stores what the entry *resolves* to, not what it literally sets. A slot it leaves unset stores the default already in force: "unset" means "no opinion", and an entry with no opinion about its token should not blank the default's.

- **A `Default` mark on the rail**, on whichever entry the current default artwork came from. Selected is what the window is showing, worn is what the character has on, and this is what the *builder* starts new entries from — a third kind of fact, so it gets its own line, quieter than the worn mark because it is true every time this window opens rather than only right now.
  - It can go stale and says so. The default is a snapshot taken when the entry was pressed, so editing that entry's pictures afterwards makes the two disagree; the mark then reads **Default (changed)** under the same amber warning triangle the worn mark uses for the same situation. Setting it again is the fix, and doing nothing is a perfectly good choice.

- **The character sheet and the prototype token open from the builder's title bar.** A build is a plan about a character and a plan about their token, and both of those are edited somewhere else — so those two are where you end up going from here. The title bar is where Blacksmith's tool windows put actions that *leave* the window rather than change it, which is the same rail the merchant's config window uses for the same reason.
  - The token opens through `CONFIG.Token.prototypeSheetClass` rather than Foundry's base class, because dnd5e replaces it — constructing the base directly would open a plainer window than the identical button on the character sheet does.
  - It is handed `{ prototype }`, not `{ document }`. A PrototypeToken is a DataModel rather than a Document, and the sheet reads the actor off it as `options.prototype.parent` while still assembling its own options — so the wrong key threw inside Foundry's constructor, before the window existed and with a stack naming nothing of ours.

- **Fill From Currently Equipped is a window, not a dialog.** Named for what it actually reads, since the two questions anybody has on opening it are "where does this come from" and "does it change my character".
  - Its rows are Blacksmith's **`blacksmith-list`** — thumbnail, title, meta, action — which is exactly this screen's row and already carries the hover, the ellipsis, the spacing, the dividers and an empty state. It had been a hand-rolled table with fixed layout, stated column widths, its own row height, hairline, scroller and ellipsis: eighteen rules where four survive, and every one of the deleted ones re-implemented something the API already did. Two of them actively fought it, and three separate bugs came out of that — two nested scrollbars, `display: flex` on a `td` emptying every column, and banding from opacity on a row.
  - A **window** rather than a dialog. It began as three tick boxes, which is dialog-shaped, and then grew a scrolling list, a live re-sort as rows move between slots, a control on every row, and a need to be resized — and `dialog.prompt` builds its DialogV2 with a title and nothing else, so resizable was simply unreachable from there. The deeper reason is the markup: a dialog takes an HTML string, so a hundred lines of table lived in a template literal where nothing checked it, which is exactly where a `display: flex` on a `<td>` hid and silently emptied every column. A window has a real template file, a footer, a remembered size, and the theme. "Map these items onto these slots" is also not only a build's problem — a merchant's stock or a loot split is the same screen.
  - It adds **no scrolling and no height of its own**. The base's window body already scrolls and `squire-tool-content` already carries the padding, the colour and the height chain — a container doing it again produced two scrollbars nested inside each other. This window keeps re-learning the same rule: its chrome is Blacksmith's, and the way to use it is to add nothing it already does.
  - **The wording says what it does and what it does not.** It reads only what is *currently equipped*, touches nothing else in the inventory, and changes nothing on the character — it fills in a plan. And it says the plan is not final: change a row here, or leave it and rearrange the figure afterwards by dragging.

- **Fill From Sheet shows you the mapping and lets you argue with it.** A row per **slot**, head to toe, each carrying the doll's own glyph, the item that landed there, and a dropdown of every slot that item could legally occupy. Everything that landed nowhere follows under a *Not equipped* rule.
  - **Slots, not items.** The first version listed the items, which showed what the character owns and hid the thing you actually need to see: which slots are still **empty**. A gap is invisible in a list of items and obvious in a list of slots. An empty row is greyed and carries no control — there is nothing to decide there, only something to notice — and you fill it from the item's own row further down.
  - The order is **stated, not derived from the doll**. The figure interleaves ring, hip, waist, hip, ring across one row because that is how a body looks; read out row by row it is an order nobody would call head to toe. The pairs belong together in a list even though they sit apart on a figure.
  - Rows **re-sort after every change**, so the table always reads in slot order and an item you just moved appears where it now belongs — including dropping into the Not equipped list when something takes its slot. Casters planning preparation get the same table for spells — Prepared or Not prepared — with a live count against the limit, since this is the one screen where it can be exceeded.
  - **One item per slot, enforced live.** Choosing a slot something else already holds turns that other one loose rather than double-booking the doll, and you watch it drop to Not equipped as it happens.
  - It replaced three tick boxes and an after-the-fact report, and the reason is what equipping does: a build takes everything it does not name **off**. The moment to disagree is before that, against a named list — not afterwards, against a doll that already happened.
...
- **Innate spells were listed among the ones a caster prepares.** The Spells panel gave At-Will a section of its own and nothing else, so a spell granted by a feature or a racial fell into its spell level — a fighter's Misty Step sat under **Level 2** as though it were a spell he chose each day, complete with a Prepared toggle he could press.
  ...
  - That test reads dnd5e's own `CONFIG.DND5E.spellcasting[method].prepares`, the same table the system's `countsPrepared` consults and the same one `canBuildPrepare` asks in the builder, so all three agree by construction. A hand-written list of methods would have claimed pact spells do not prepare, which dnd5e says they do.

- **A character with a spell but not a caster's shape could not plan a prepared list.** The prepared column appeared only for someone with a preparing class or spell slots — both inferences about what *kind* of character this is, and both missing the same case: a fighter carrying Misty Step from a subclass, an item or a feat had nowhere to plan it, because nothing on their sheet said they were the sort of person who prepares anything.
  - It now also asks the direct question: **does this character own a spell a build could prepare?** Same test `applyBuild` uses to decide what it may set, so the column appears exactly when there is something to put in it. A cantrip or an innate spell still answers no — neither is ever chosen — and nor does a domain spell that is always prepared.
  - **And the limit follows the same rule.** It was the sum of `preparation.max` across preparing classes, which for a character with none came to **zero** — putting all twenty-six cells past the limit, dimmed to a fifth and refusing drops, so the column would have appeared and been unusable. Where dnd5e has an answer it is still the only one that counts; where it has none, the ceiling becomes the number of preparable spells the character owns. That constrains nothing they could have done anyway, and the grid then shows as many live cells as there are spells to fill them rather than twenty-six empty ones for a character with two. The GM set that number by handing the spells over.
...
- **The import window ignored every dropdown you changed.** Picking a slot for an unmapped item left it unmapped and did not evict whatever already held that slot. The listener matched `.squire-import-slot`, which had been renamed to `.squire-import-slot-select` in the markup to stop it colliding with the row's leading icon column — that column keeps the old name and is a *sibling*, so `closest()` walked past something it could never reach and returned null.
  - **It failed in the worst direction available.** A `select` shows its own selection with or without a listener, so the window looked like it had taken the change; but `answer()` reads the assignment map the dead handler never wrote to, which means **Fill Build imported the original plan and silently discarded every correction made to it**. A visibly broken dropdown would have been better than one that looked like it worked.
  - Both control names are now single constants shared by the markup, the listeners and the reads, because there is no way to notice when a class name and the selector that hunts for it stop agreeing.
...
- **Open Prototype Token now checks for a parent, not just for a token.** The sheet reads the actor off the prototype as `options.prototype.parent` while it is still assembling its own options, so a parentless one throws inside Foundry's constructor with a stack that names nothing of ours. Asking here costs nothing and produces a sentence instead.

- **The build selector jumped back to the top on every click.** Selecting a build redraws the whole window, and a fresh DOM starts scrolled to the beginning — so with more than a handful of builds, choosing two in a row meant scrolling back down to a spot you had just been looking at. The rail's scroll position is now carried across the redraw through Blacksmith's own `_saveScrollPositions` / `_restoreScrollPositions` hooks, which the rail needs an entry in because it is a second scroller inside the tool body the base class already tracks.
  - Those two hooks look dead from the outside: `window-base.js` calls them as `this._saveScrollPositions()`, so a grep for either name finds no caller, and the first version of this fix hand-rolled the same behaviour in `_preRender` beside them for exactly that reason. Going through the base also buys the `requestAnimationFrame` the hand-rolled version did not have — the difference between writing `scrollTop` after layout and writing it before.
- **"Not equipped" was factually wrong**, and the items it named were the proof: every one of them *is* equipped on the character. They are simply not **mapped** to a slot in this build. The section, the dropdown and the intro all say mapped now, and the intro says what it costs — an unmapped item stays equipped but is not part of the build, so equipping the build later takes it off.
- **A row that landed nowhere says why.** Three reasons with three different fixes, indistinguishable from the outside until now: *nothing could work out where this goes*, *belongs in Neck — already taken*, and *not read as something worn*. The third is the one that made an Amulet of Health an unanswerable question three times running: dnd5e types some wearable magic items `consumable`/`trinket`, which the classifier reads — correctly by its own rules — as "not body equipment at all", the same answer it gives a potion. Verified: the same item as `equipment`/`trinket` or `loot`/`trinket` resolves to the neck; as `consumable`/`trinket` it resolves to nothing. The dropdown still offers every slot, so it was always mappable by hand; what was missing was any way to know that.

- **The slot is a column now, first on every row.** It was the row's *meta* line when something occupied the slot and the row's *title* when nothing did — so the one thing you scan the list for moved between two positions depending on whether it had been filled, which is the opposite of scannable. Every row carries the same three columns in the same order: slot, picture, name, control, with the picture's space held open on an empty row so nothing shunts left.
  - This is the only structural addition to `blacksmith-list-row`, and it is one the API genuinely lacks: its row has a single image zone, and this screen needs two marks — what the slot *is*, and what is in it.

- **A dagger goes in the sheath, not the off hand.** dnd5e gives a light weapon and a shield the same grip, and this took that to mean "off hand" for both — but for the light weapon it is a dagger: the thing on your belt, not the thing in your other hand. Most characters carry one primary weapon and a sidearm; the off hand is for actually fighting with two, which nothing here can know. A second light weapon still reaches the off hand once the sheath is full, which is the dual-wielder's case falling out of the same rule.
- **The proposal now respects the slots' own rules.** It checked that a slot existed and was free, but never that it would accept the item — so a candidate list could propose something the doll itself would refuse, and a shield landed in the sheath the moment light weapons started preferring it. Two placement loops had drifted apart on this; both check now.

- **An unarmed strike is ranked last and can never be sheathed.** It was taking a hand ahead of real weapons and being put in the sheath, which is storage — there is nothing to store. It is not excluded, because punching someone is a real way to fight and belongs in a hand; it is simply the last thing that should get one.
  - **The rule is physicality, not a name.** dnd5e types some innate weapons `natural`, but the classic `items` pack types Unarmed Strike `simpleM`, exactly like a dagger — so the subtype alone gets it wrong in half the worlds in existence, the same pack-dependence that made the clothing rule wrong. A thing you can carry has mass and a thing you can buy has a price; something with neither is not an object at all, whatever the compendium called it. Checked against every weapon in both packs: **359 of 364 have both**, and the five that do not are two Unarmed Strikes, a Clawed Gauntlet and a template entry. No false positives on real gear.
  - Natural weapons are no longer excluded from builds outright — that hid them rather than ranking them.

### Added
- **The system's item card on every row of the import window.** The screen asks where a Rod of Lordly Might should go, and nobody can answer that from a name — the card is what makes the question answerable. Reapplied after each redraw, because the rows are replaced whenever something moves between slots.

- **The import table's cells were `display: flex`, which took them out of the table.** A `td` set to flex stops being a table cell: it leaves the column model, widths stop applying, `colspan` stops working, and every column collapses to its own content — so the item names and pictures were rendering the whole time with nowhere to be, and the dialog looked like it had pulled in no data at all. Each cell keeps `display: table-cell` now and does its layout in a div inside.
- **The import list scrolls, and the table has a stated width.** A character with forty equipped items produced a dialog taller than the screen with its own buttons pushed off the bottom. The list scrolls inside a fixed height; the heading and the controls do not. The width is stated rather than automatic, because three columns of known size should land in the same place on every row.

- **Copy-pasted tokens switch the tray again.** Drop one monster, copy it four times, and selecting a different copy left the tray showing the first — and clicking them in the Party tab did nothing either. An unlinked token's actor is *synthetic*, built from the base actor plus that token's delta, and it carries the **base actor's id**: four pasted goblins report four different uuids and one identical `.id`. Seven comparisons across the module asked identity with `.id`, which is a question about the *kind* of creature rather than the creature.
  - Both symptoms were one bug. The Party panel was already right — it keys on `token.id`, which is genuinely unique per copy, and calls `token.control()`. The click selected the correct token and the tray then refused to follow, because the `controlToken` path compared actors by `.id` and returned early. When two entry points fail together, the shared gate downstream is the thing to look at.
  - Also fixed by the same sweep, none of which had been reported: handing an item from one copy to another was read as a self-drop and silently did nothing; favouriting on one copy repainted the panel while a different copy was on screen; and resolving "the token for this actor" returned whichever copy came first in the controlled set rather than the one being shown.

- **A prepared spell past the limit was unreachable.** Cells beyond what a character can prepare are dimmed and inert — correct for an empty one, and wrong for a full one, which happens whenever the limit falls or a build outlives a level. The spell rendered at 22% opacity with pointer events off: it could not be read, hovered or right-clicked away. A filled cell is now fully legible and fully live wherever it sits, marked amber if it is past the limit.
- **Prepared spells show the system's item card**, like every other filled slot in the window. They carried `data-item-id` where dnd5e's tooltip layer looks for `data-item-uuid`, so they were the one filled thing here you could not hover to read.
- **One list of what a build never touches**, used by the importer, by applying, and by drift alike. It was written out twice and the copies had diverged: siege and improvised weapons were excluded from classification but not from the unequip sweep, so equipping any build would have unequipped a ballista. Natural weapons, siege and improvised weapons, and containers — and nothing else, because being unplaceable is not the same as being ignorable.
...
- **Pull From Sheet**, beside that switch: fills the build from what the character has equipped and prepared *right now*. The answer to setting a kit up on the sheet before finding this window, and to the commoner case of changing something there and wanting the build to catch up. Two checkboxes, gear and prepared spells, because they are the same two decisions.
  - A third checkbox, **Empty the build first**, clears everything including the parts not ticked above. Each half an import touches is already replaced rather than merged, so this is only for the half it does *not* touch: taking gear alone into a build that also plans spells otherwise leaves the old list sitting there — correct when you meant to keep it, surprising when you did not. It can be ticked on its own, which is simply "empty this build". The name and the pictures survive either way; those are what the build *is*, and this empties what it holds.
...
- **`Weapons on handle` is now `Actions on Handle`, and only appears on the Builds tab.** What it puts there depends on the character — a martial's three weapons, a caster's three spells — and one word true of both beats a label rewritten per character to stay honest; the tooltip still names the actual three. It is hidden on the other tabs because a costume equips nothing, so there it was a switch for something you were not looking at.
...
- **The handle strip is the doll's big three, and nothing else.** A martial gets Main Hand, Both Hands and Off Hand; a caster gets their Primary, Secondary and Tertiary spells. Same three slots the doll gives that character, so the strip shows what their build is actually built around rather than a fixed idea of what a build is for — a wizard has no business keeping a greatsword to hand, and now gets the three things they will actually reach for.
  ...
  - **The toggle is named for what this character would get** — "Spells on handle" for a caster, "Weapons on handle" for everyone else, with the tooltip naming the actual three. One global setting still, but a wizard reading "weapons on handle" would reasonably conclude it was not for them and leave it off.
  - It follows the DOLL's caster test (the class's spellcasting progression), not the prepared-column one — otherwise it would promise weapons to a caster whose build happens not to plan a spell list.
...
- **`Set Artwork as Default` is now `Reset Default Artwork`.** It and the new entry above it both set the builder's defaults and differ only in where they *read* from, which is the whole difference between them and was the part you had to guess from two near-identical names. Named for their source now: **Set As Default Artwork** takes the selected entry, **Reset Default Artwork** takes whatever is on the character sheet. Resetting also clears the `Default` mark from any entry that held it, since the answer no longer comes from there.

- **The builder's default artwork is no longer frozen at first open.** It was captured once, before any build could touch it, and then trusted forever — which was right while nothing could set it deliberately. Now that an entry can be made the default, that flag holds the *current* default rather than the original face. The way back to their real face is unchanged and is the thing it always should have been: the auto-created **Default Costume** build, which a player can see and click, rather than a flag nobody can point at.
- **The selected build uses the party roster's selected and hover treatment** — its green border and dark ground, and the roster's orange hover — instead of a coloured left edge private to this window. A card you click to choose is the same gesture wherever it lives.
- **The scale readout follows the slider while it is dragged.** It only moved on release, which is the one moment you no longer need it. The value written still waits for release: `input` fires per pixel, and a flag write per pixel would re-render the window out from under the control being dragged.

- **The builder opens on what the character is WEARING**, not on whichever build happens to sit first in the list. That fallback picked a build by nothing except its position, so the window opened describing a plan the character was not on and a doll full of gear they were not in. What is worn is the one build that is a fact rather than a guess.
  - If nothing is worn, **nothing is selected** and the workspace says so — *Nothing Selected · Choose a build or costume from the left*. Picking one on the player's behalf would be inventing an answer where there is not one.
- **A build on the tray handle wears the same face as its tile**: the spell a caster leads with, the weapon anybody else does, a costume's own picture. It was the build's picture in one place and its headline item in the other, which is two answers to "what does this build look like". `resolveTileImage` is now the single one, and both read it.

- **Convert to Costume / Convert to Build moved to the right-click menu**, and the switch beside the name is gone. It sat in the most prominent place in the window for an operation nobody does twice, and put there it implied the two modes were a setting on one thing — while New Build and New Costume make that choice up front, which is when it is actually made. The header band is the build's name alone now.
  - **Converting discards the other mode's contents**, and says so first. Keeping them was the obvious thing to do, and it is what produced the ghosts: a costume went on carrying invisible gear, its tile drew a weapon nobody could see the source of, and converting back handed you a build full of whatever was in it weeks ago. What conversion is actually worth is the **name and the pictures** — the slots were always a few drags — so it keeps the expensive half and drops the cheap one, and the result no longer depends on which direction you came from.

- **A build's tile is faced with its headline choice**: the spell a caster leads with, the weapon anybody else does, and for a costume its own picture — which is the whole of what a costume is. It falls back to the build's picture when that slot is empty.
...
- **An item dropped on the tray shows you where it went.** The column opens on the sheet at **All**, scrolls the new row into view and pulses the whole row six times — the quest-entry highlight the map pin used to draw, brought back for the item that just landed. A drop lands somewhere you may not be looking — a longbow dragged in while the Spells tab was open used to arrive with no visible sign it had arrived at all — and the toast tells you WHAT was added without telling you where to find it.
  - **All, rather than the tab that owns the item.** A drop is not a navigation: you were looking at something when you did it, so the least surprising landing is the one view where the new row is visible whatever it turned out to be, instead of being moved to Spells because the thing you dragged was a spell.
  - Best-effort, and deliberately quiet when it cannot deliver: if a search term or a filter chip is hiding the new row it stays hidden, because clearing a filter you set on purpose to show off an animation is the worse trade. The toast still says what arrived.
- **The tray's own compendium search is gone; the magnifying glass opens Blacksmith's palette instead.** It sits with the broom and the builds icon, on the actions side of the separator, because it opens a window rather than changing what the column shows — and being a mode was half of what was wrong with it. Squire's version took over the panel stack, so finding something and looking at what you already have were mutually exclusive; you pressed a button and then went hunting for where the item landed, instead of dragging it where you wanted it; it searched less and found less; and two compendium searches a click apart left the honest question of which one to use. Blacksmith's is a window beside the tray with facets, a browse mode, and native drag onto the sheet.
  - **Nothing was given up to delete it.** The rules it used to enforce moved out of it in the same release and now govern every path content can take onto a sheet, including the drag out of Blacksmith's palette. The search left; the gate stayed.
  - **The *Look only* rung is gone**, folded into *Off*. It permitted searching and reading in Squire's own column and nothing else — Blacksmith's palette is a player's to open from their own menubar, and not Squire's to gate — so it had become *Off* with a longer name. A world set to it moves to *Off* on load.
  - The *Clear search and keep open on add* option went with the column it belonged to.
- **Adding content to a character is governed in one place now, whatever door it came through.** The four rungs of *Let Players Add Content* — off, look only, ask the GM, add freely — used to be checked by the tray's own compendium search and by nothing else, so the rule a GM set governed the one path that happened to be ours. Every acquisition runs through the same gate: the compendium panel, a drop on the tray, a drop on a party card. A player on **Ask the GM** now gets the request card for a world item dragged out of the sidebar exactly as they do for a compendium spell, and a player on **Look only** gets a refusal that says which rule stopped them rather than nothing happening.
  - **Transfers are deliberately untouched.** Handing an item from one character to another is a different question with its own setting and its own approval cards; routing it through here as well would make a player on Ask the GM need two approvals for one handoff.
  - The setting is renamed on screen because it no longer only means compendiums. The stored key is unchanged, so no world loses its choice.
- **One chat card for anything arriving on a sheet from outside.** There were two, and they disagreed: a drop on a party card announced that *the GM* had added the item whoever had actually done it. The card names the user now, and it is posted for player acquisitions only — a GM stocking a character out of the compendium search does it many times in a row, and a card each would bury the log under a record of what the GM already knows they did.

### Fixed
- **The import report is in the window, not in notifications.** It was a fading toast, which took the list of names away before anybody read them; then two permanent ones, which is a stack of warnings over the corner of the screen. Both were the wrong place. These are things to **drag onto the figure**, so the list belongs beside the figure — where you can read a name and place it without the notice going away when you click. It sits over the foot of the doll with a dismiss button, and clears itself when you select another build, since it describes an import into the one you just left.
- **Weapons go where merit says, not where the sheet's ordering says.** Placement followed whatever order the character sheet happened to list things in, which put an Unarmed Strike in the main hand and an Oathbow in the sheath. Contested slots now go to what the character actually leads with, ranked by four signals in the order they deserve weight: **favourited on the sheet** (the player saying so in their own words), **on the tray handle** (the same statement made with a drag), **damage** (what separates an Oathbow from an Unarmed Strike when nothing else has an opinion), and **magical or attuned** as a tiebreak. **Armour outranks a garment for the chest** by a margin no amount of dice can overturn: a Robe of the Archmagi and a suit of studded leather both claim it, a character has one chest, and the armour is what the window's AC badge is reading — losing it to a robe on a tie in list order was the worst of the two outcomes. The classifier was never wrong about these — both are weapons and both can be held — so the question was never "where could this go" but "which of these do they lead with", which is occupancy and ours.
- **Containers are never given a slot, and never unequipped.** Blacksmith answers `back` for a backpack, which is a fair general answer; this doll declines it, because a slot spent on a Fanny Pack of Holding says nothing about how a character fights or what they look like, and the back is one of thirteen places a cloak could have gone. Applying a build now skips them for the same reason it skips natural weapons — no build names one, so without this, equipping anything would take the character's pack off their back. Their contents need no special handling: an item stowed in a bag is not equipped, so it was never in scope.

- **The doll's shape follows the CLASS, not whether the character prepares anything.** A ranger prepares spells and is not a spellcaster, so they were handed a wizard's doll with three quick-cast slots where their weapons belong. dnd5e already grades this on the class item — `full` and `pact` are casters, `half` and `third` and `artificer` are martials who also cast — and that grading is exactly the distinction this layout needs.
  - **The preparation switch asks a different question**, and a looser one: does this character have anything to prepare at all. So a ranger, a paladin and an eldritch knight all get the switch on a martial's doll, which is right for every one of them. In modern rules half the martial classes cast something, and having spells says nothing about what a character leads with.
- **The importer leaves an item alone rather than inventing a home for it.** It used to force every equipped item into some free slot, so a doll filled up with arrows on the arms and a kit on the feet — confidently wrong, which is worse than plainly incomplete. Anything it cannot place is now named back to you so you can drag it in yourself. *Wrong imports are worse than none.*
  - **Two failures, reported apart.** *Nothing could say where this goes* is a gap in what anything knows, and you fix it by dragging the item in. *The slot was taken* is not a failure at all — a character can equip two robes on their sheet and has one chest — and rolling the two together made the second look like a defect. A better classifier makes collisions the commoner case, not the rarer one.
  - **Two empty answers, kept distinct.** A potion, a scroll, a natural weapon are *not body equipment* — there is nothing to decide and nothing to say, so they are silent. An item whose home is merely unknown, or whose slot was already taken, is *named*. Collapsing the two means either nagging about every potion or silently dropping the thing somebody actually wanted.
  - **Classification moved to Blacksmith**, as `api.equipLocations.resolve()`. Squire deleted its vocabulary table, its property helper and every structured rule; what is left is fifteen lines mapping a body location onto a slot on *this* doll. That division is the point: knowing a ring goes on a finger is classification and belongs to the suite, knowing this character already has one on is occupancy and belongs here — and Vault, Merchant and Blacksmith's own importer would otherwise each have written the first half differently.
  - Squire's slot keys were deliberately kept OUT of the shared contract. `sheath`, `hip1` and `bothhands` are positions in this window's layout; the API answers with thirteen generic body locations and this file turns them into the twenty-one keys the doll has. `sheath` needs no location of its own — it falls out of a light weapon's grip.
  - Both entry points **degrade rather than fail** on an older Blacksmith: the import button says so and does nothing, and the first-run gear snapshot skips silently. Without that, every item would come back unplaceable and read as a broken importer rather than an out-of-date dependency.

- **Importing places things in TWO PASSES.** Everything with a claim on a particular slot is placed first, and only then do the claimless take what is left. In one pass an arrow reached the neck slot before the Amulet of Health was considered at all, and the amulet ended up on the character's feet — not because the guess was wrong but because the guesses were made in the order the items happened to be listed. That ordering was most of what looked like random stuffing.
  - **dnd5e models no body slots and never has.** `miscEquipmentTypes` is clothing, ring, rod, trinket, vehicle, wand, wondrous; there is no data anywhere saying a helm is worn on the head, which is exactly why the doll's body slots accept anything. Blacksmith has nothing for it either. So the engine asks the data everything it can answer — weapons, ammunition, shields, armour values, rings, containers, tools — and reads the item's name for the rest, which is the only evidence that exists.
  - **Natural weapons are excluded outright.** Claws and unarmed strikes are a fact about the creature rather than something it picked up, so they spend no slot — and applying now refuses to unequip one, since a build never names them and would otherwise turn them off. Drift skips them for the same reason: counting them would report every character with claws as permanently drifted from every build.
- **Importing places things where they belong.** It asked the data four questions and then dropped every leftover into the first free body slot starting at the head, which is how a fanny pack ended up on somebody's scalp and arrows on their feet. It now asks the data everything the data can answer — weapons, ammunition, shields, anything with an armour value — and then reads the item's **name** for the rest, which is the only evidence there is: nothing distinguishes a helm from a boot in the data, which is precisely why those slots accept anything. Leftovers start at the pouches rather than the head.
  - Off hand before both hands for a second weapon: a sidearm is far commoner than a greatsword, and Both Hands claimed for a shortsword is the one placement that says something wrong about how you fight.
- **The drift count sits in the portrait's bottom-right corner** instead of across the middle of its face. It carried no position at all, which for an absolutely positioned box means "wherever you would have been in the flow".

### Added
- **Gear can be dragged from one slot to another**, swapping with whatever is already there. Fixing a placement — after an import, or after changing your mind — cost clearing one slot and finding the item again in the tray. The destination's rules still apply: a torch dropped into the ammunition circle is refused whether it came from the tray or from the character's own belt.

- **The compendium access migration ran before the setting it migrates existed.** It was called from one `ready` callback and the settings are registered in a later one, so it reached the registry too early. It never surfaced because the first thing it did was a raw storage read that needs no registration and returns nothing on a world with no legacy value -- so it took an early return before touching anything. A world that actually held the old `compendiumAddPlayers` boolean threw into its own catch and silently kept the old value, which is the case the migration exists for. It now runs immediately after the settings are registered.
- **Dropping an item on the tray or a party card no longer bypasses the player content rules entirely.** Both drop handlers created the item on the strength of "a character is selected" — so a player on any rung below *Add freely*, including **Off**, could drag the same document out of a compendium browser, the Items sidebar, or another sheet and put it on their character anyway. The gate was on the one door and the wall beside it was open.
- **A drop on someone else's party card says so.** It refused further down as a raw Foundry permission error, or silently; it now refuses at the drop with a toast naming the character you do not own.
- **The legacy Item Directory drop resolved nothing.** It looked the item up by id using a uuid, so it never matched — harmless only because the modern sidebar sends a payload that lands elsewhere.
...
- **The builder opens in the dark theme.** The base class defaults every tool window to light, which is right for the ones that are mostly text — this one is a wall of artwork, portraits and tokens and item pictures at every size, and dark is the ground that lets them be the bright thing in it. A default and not a lock: the titlebar's theme control still works and the base remembers whatever is picked, under a named key so a class rename cannot orphan that choice.

- **Set Artwork as Default**, on a build's right-click menu, records the character's current portrait and token as the artwork every unset image slot falls back to. The defaults are captured once, the first time the builder opens, and then trusted forever — which is right for what they are for, since a costume overwrites `actor.img` and a record that followed it would stop being a record of anything. What they cannot survive is the **file moving**: the flag then names a path to nothing, every unset slot falls back to that same dead path, and one renamed file makes the whole window look broken.
  - It asks first, and the confirmation names the hazard rather than the action: this reads what the character looks like *right now*, so doing it while a costume is on would record the costume as the original — the one thing the defaults exist to prevent.

### Fixed
- **A dead image path no longer renders as a broken picture.** Any image that can come from the captured defaults falls back once to the character's live artwork, and hides itself if that fails too, leaving the slot's own glyph showing. The browser's error event is the only cheap way to learn a path has gone stale — knowing otherwise means an async file browse on every render.
  - It deliberately does **not** heal the flag. A transient failure would overwrite a good default with whatever the character happens to have on, and if that were a costume it would record the costume as the original. The repair is the deliberate menu action above.

- **The top of the window is a header band.** The build's name and the prepared column's heading share one ground and one height across row one, so the rule between them reads as splitting a bar rather than as a line that happens to pass two captions. That row was the emptiest part of a window whose middle is dense.
- **The prepared column is titled "Preparation"**. It sits in the header band rather than inside the column, because above the cells it would push every one of them half a row out of line with the doll.
- **The prepared cells stay square, and the column ends fifteen pixels short of the doll.** Filling that gap was tried and reverted: stretching the grid to the full height made the cells 40.2 tall against 39 wide, which traded a visible gap at the foot for a pixel of error in every row — by row thirteen the drift was the same fifteen pixels, spread through the middle where it broke the alignment instead of sitting harmlessly at the bottom. Only the first row still lined up with the body beside it. The remainder at the foot is the honest leftover of a grid that matches the doll exactly.
- **The Build/Costume switch leads the title row**, with the name after it. It is the more consequential of the two — renaming is cosmetic, and this decides what wearing the thing does — and it is a fixed width, so putting it first gives the name the rest of the band however long the switch's labels run.

- **Empty slots say what belongs in them**, instead of "drag an item here" — which told you the gesture and nothing else, and only after you had already worked the gesture out. Each slot now names the kinds of thing that go there (*Helms, hats, circlets, crowns*), and every one of them ends with where to drag from: **the Squire tray or your character sheet**. One tooltip answering both questions at once.
  - The body slots are worded as **suggestions**, because that is what they are: nothing in the data distinguishes a hat from a boot, so those slots accept any physical object and always have. The ones that *do* enforce something — the sheath, ammunition, the quick-use slots — are worded as the rule they actually apply, since there a refusal is possible and "why did nothing happen" is a real question.
  - The prepared cells got the same treatment, including the dimmed ones: they now say what they are (*Not yet — the cell is here so you can see what levelling up will buy*) rather than only what they are not.

- **The Sheath is round**, matching the ammunition beside it. It was square on the argument that a sheathed weapon is wielded while ammunition is only spent — but in the row as drawn those two are the pair that is neither worn nor held, and a matched pair of circles says so better than a distinction nobody was reading.

- **Worn tells the truth now.** The `activeBuild` flag records which plan was last applied, and nothing ever re-checked it — so equipping something from the character sheet, or editing a build after wearing it, left the tile claiming a state that was no longer true. A build's slots are now compared against what the character actually has equipped and prepared, and the tile drops to **Last worn**, with a warning, when they have parted company.
  - **The flag stays as the record, and the check only qualifies it.** Which plan is in play still has a right answer when the character has drifted, and it is what the handle's weapon strip genuinely wants. Deriving "worn" outright instead would let an empty build match a naked character and let two identical builds both claim it; clearing the flag on mismatch would silently empty the handle strip the first time somebody picked up a torch.
  - **"Last worn", not "modified".** The first wording claimed something the check cannot see: the plan and the character disagree, and nothing here knows which of the two moved — you unequipped a sword, or you edited the build after wearing it. "Last worn" says only what is certain. The full **Worn** is kept for the case that passes, because there the character genuinely is wearing it and a label should be as strong as its evidence.
  - **The mark goes on the thing that moved.** The tile says *that* it happened; each slot holding an item that is not on the character gets an amber warning and a dashed edge, saying *where*; and a plate on the portrait counts what the character has on that this build never named, since there is no box on the doll for an item the plan does not mention. Amber and not red — drifting from a plan is a normal thing to do.
  - Marked only on the build being **worn**. On any other build every slot would be "not equipped" and the mark would mean nothing.
  - Compared with the same test `applyBuild` uses, so the two cannot disagree about what counts as equippable or as counting against a preparation limit. Portrait and token are deliberately *not* compared — artwork is not equipment, and a costume worn over a build changes them by design. Attunement is not either, since applying never sets it.
  - The whole rail costs one walk of the sheet rather than one per tile: the equipped and prepared sets are read once and every build is compared against them.
  - Known limitation: this window keeps no hooks on the actor, by design — a flag write that fires `updateActor` re-renders the entire tray for one slot. So equipping from the sheet while the builder is open will not repaint it. The mark is right on open and after any interaction with the window.

- **A costume sets how the token is DRAWN, not just what it is drawn with**: its dimensions in grid spaces, the image fit mode, and the scale — Foundry's own three token-config settings, applied along with the artwork. The picture was only ever half of what a costume changes: a giant in disguise is not the same size on the map, and a portrait cropped tall wants a different fit from one cropped square to sit right in the same box.
  - **A gear build never touches any of it.** Equipping a breastplate does not change how many squares you stand in, and a build that reset your token's size would be deciding something that belongs to the character rather than to the kit. It uses whatever the token already has.
  - **All three on one row**, because they are one decision — how big the token is, how the picture is fitted into that, and how large it is drawn inside the fit. Stacked they read as three unrelated settings.
  - **The dropdown's tooltip lists what every fit mode does to the scale**, all five at once rather than only the one currently chosen — the point of the note is deciding between them, and a description that changes as you change the thing it describes cannot help with that. It was a permanent line of italics under the whole row, which is a lot of standing furniture for a fact about one control.
  - The three captions **sit on one line**. The row was bottom-aligned, so it hung off whichever control happened to be tallest: a range input is shorter than a text box, which left the scale's caption floating clear of the other two with a gap under it that looked like padding nobody asked for. Every control row is a stated 22px now and the row aligns to the top.
  - The warning mark is **sized to the caption it sits in**. At 0.9em of a 0.66em label it was bigger than the words beside it and off their baseline, which reads as a mark that has landed in the wrong place.
  - Every control can be put back to **no change** by clicking its warning mark. That was a right-click on the field, which nobody would guess at and nothing announced; the mark is already the thing saying "this will act on the character", so clicking it to stop that is the obvious reading — and it is a visible target rather than an invisible gesture over an area holding three live controls. Clicking it clears the whole field, since Dimensions is two boxes saying one thing and half of it is a width with no height.
  - **Each control is as wide as what it holds.** They were equal thirds, so two single-digit boxes and a dropdown whose longest entry is "Full Height" were stretched across a third each and read as fields expecting a sentence — and the scale's caption floated over the middle of nothing. The slider takes the remainder, being the one control here that is genuinely better long.
  - **The warning follows its label** instead of leading it. In front it pushed the words sideways the moment a control was set, so the three captions stopped lining up with each other exactly when one of them began to matter.
  - The warning tooltips **say what will happen and how to stop it** — "will resize the token to 2 x 2 grid spaces / click here to leave the size alone" — rather than naming the control, which the caption beside them already does.
  - **Only what CHANGES is marked**, with an amber dot. The first version marked the opposite: a "no change" tag on every untouched control, which put three notices on the commonest state and none on the one worth noticing. The same inversion went through the image slots, which carried a "no change" caption for the same reason and now carry the same dot when they will actually set something.
  - It is a **dot and not a caution triangle**, which was wrong twice over. Nothing here is an error — there is no problem to fix and nothing broken — and the triangle is already how this window says a real one: a slot whose item is not equipped, a build the character has drifted from. A dot says *active*; a triangle says *wrong*; they cannot be the same glyph. The tooltips say it in those terms too: "**On.** Wearing this costume sets the image fit to Fill. Click to switch it off, and the token keeps whatever fit it already has" — rather than the previous "leave the fit alone instead", which named neither what was happening nor what stopping it would do.
  - **The token controls sit under the portrait and token circles**, above the costume's own picture. They describe how the *token* is drawn, and under the main picture they read as settings for that — the one image in the view they have nothing to do with. The build's own picture lost its caption outright: it is never written to the character, so there was no change to announce either way.
  - The **Default Costume** now records the token's own size, fit and scale when it is created. It is the costume that means "back to normal", and one that changed the picture while leaving a previous costume's dimensions in place would not get anybody there.
  - The fit modes are read from **Foundry's own `TEXTURE_DATA_FIT_MODES`** rather than listed here. They are core's to change, and a hardcoded list fails silently — a mode nobody can pick, or one written into a flag the token document then rejects.
  - One canvas write carries the artwork and the geometry together rather than one for each, and only the fields that actually differ on each token: they describe the same token, and somebody watching it should see it change once.
  - Undo puts all of it back. The previous geometry is read before the first write whether or not the costume touches it, since undo is cheaper to make unconditional than to make conditional on a record of what was written.

- **The build rail scrolls**, and the tiles are **square**. A list of arbitrary length was making the window taller instead — the one thing a list like that must never do. Only the list scrolls: the buttons and the options stay put, because a New Build button you have to scroll to find is a button nobody presses. Square because everything else in this window is, and because the portraits people choose for a build are portraits: a short wide band shows the middle of whatever was picked rather than the whole of it.

- **New Build and New Costume, side by side.** Two buttons rather than one and a switch to throw afterwards: you know which of the two you are setting out to make before you start, and the switch beside the name is for changing your mind. Neither is the main one, so they are equal halves of the row.

- **Casters and martials get different dolls.** The layout was one shape for everybody and that shape was a fighter's: three big slots at the bottom for main hand, both hands and off hand, with anything a caster does pushed off to the side. So a wizard's window led with the three weapons they do not use. A character who prepares spells now gets the big three at the bottom for **quick-cast abilities** — primary, secondary, tertiary — with their weapons as the smaller slots in the row above, and their prepared grid directly over them. A martial keeps the weapons big and has no such column at all. Same window, same rhythm, opposite emphasis, decided by whether the character prepares anything.
  - Both layouts share their slot keys where they mean the same thing, so switching (multiclassing into a caster, say) never strips a slot: `mainhand` is `mainhand` on either doll. `BUILD_SLOT_KEYS` is a de-duplicated union of every layout, which is what makes a build's stored slots outlive a change of shape.

- **The prepared column**: twenty-six cells, two across and thirteen down, beside the doll, holding what a caster has prepared. It briefly held **carried potions** for everybody else, and that was cut: everything else in this window is a claim that Apply *enforces*, and there is no carried state in the system to set — so those cells looked exactly like the ones around them and did nothing. A martial's build is the doll. Their window is narrower by this column rather than padded out with something inert.
  - **Its size is not a choice.** The workspace is six tracks: the doll spans five and the column takes the sixth, with the body grid's own 6px gap between them. So the doll's five columns divide back to exactly one track each — a gear slot *is* a track — the column is one track wide, and its two cells share that same 6px gap. Four cells in a two-by-two therefore come to exactly one gear slot: you could drop them into the neck box and they would fit it. Two stack into one body row, so the two grids share every horizontal line down the window. Twenty-six is what thirteen of those rows comes to; the number fell out of the layout rather than being picked.
  - **The column starts one row down**, level with the top of the neck slot rather than with the portrait circles. The doll's first row is the head and the two images, which the pack has nothing to sit beside — starting up there put every cell half a row above the slot it should have lined up with. A square at the head of the column supplies the offset, and because a square one track wide is one track tall there is no hard-coded number doing it.
  - **No heading and no count on the column.** They were the first thing to go once the alignment was right: a label above the grid pushes every cell out of true, and it was telling a cleric that they are a cleric and counting cells they can see. The spell slots fill that square at the head instead — the one place in the window they cost nothing.
  - **Every size in the window comes from four numbers now** — a track, a gap, a gutter, and the doll's height derived from them — and they are fixed pixels rather than fractions of the window. While the tracks were `1fr` the slot size depended on how wide the window happened to be, so a caster's doll and a martial's came out different sizes and their windows came out different heights; the two widths would only have agreed at one exact window size, which is a coincidence rather than a layout. Fixed, the doll is the same object in both, and a window a few pixels wider than it needs is a few pixels of space rather than everything inside it quietly resizing.
  - **Two widths, and only two**: the doll, plus the prepared column for those who have one. This is not the resizing that came out earlier — that was one window changing size under the cursor when a switch was thrown. A fighter's window is simply a different window from a wizard's, and neither ever becomes the other. Sharing one width would have paid for a column half the characters do not have by shrinking every gear slot on the other half.
  - **A rule between the doll and the column**, mirroring the one between the rail and the workspace, and running the full height past the title the way the rail's does. It gets a track of its own rather than a border or padding on either side, since either would eat into a column the rest of the layout is measured against.
  - **The title and the Build/Costume switch are part of the build area**, not a band across the whole window: they are the first row of the same grid, so they stop where the doll stops and the rule stands beside them.
  - **The height never changes, for anyone, in any mode.** The workspace's second row is pinned to the doll's height, so a costume is exactly as tall as a build and both are the same for every character. Toggling used to give a different size than opening straight into costume mode — two paths to the same state that did not agree, which is what it looks like when a size is being derived from whatever happens to be on screen.
  - **One base width, and one exception.** A martial's build, a martial's costume and a caster's costume are all exactly the doll. Only a caster's *build* is wider, by exactly the prepared column, so switching a caster to costume takes the window back down rather than leaving it stretched around empty space. The width is re-derived on every render, which means the mode switch, picking another build from the rail, and opening straight into a costume all reach the same answer without any of them having to remember to.
  - **The frame is measured, not guessed.** Everything inside is laid out in fixed pixels, so the width the content needs is known exactly — but the chrome a theme draws around it is not. Rather than guess once and live with the slack, the rail and the workspace are measured against the room they were actually given and the difference is handed back. The constant only has to be close enough to render once.
    - It runs **a frame after the render, not inside it**. ApplicationV2 applies the position it was constructed with *after* `_onRender` returns, so a width written inline was overwritten by the opening guess on the first render and only on the first render — every window opened at the wrong size and then snapped right the moment anything made it render again. A frame later puts the measurement after the position pass instead of before it, on every render alike.
    - The measurement is **relative**: the slack comes off the width the window *has*, not the width it *wants*. Those two are only equal when nothing has changed, so taking it off the target over-corrected by the whole width of the prepared column on a mode change and then oscillated between too wide and too narrow on alternate renders — a strip of dead space one way, a horizontal scrollbar the other. Measured against the current width it converges in one pass from anywhere and needs to know nothing about which mode it is in.
  - **The rule beside the doll sits in the same 21px the rail's does** — 10px, the rule, 10px. Its track was 21px on top of a gap either side, which put 16px of clearance on each side and made the right-hand division half again as wide as the left.
  - **One gap everywhere in the doll.** There were four different spacings in a single column — 6px inside the body, 8px between the weapons, 10px of margin under the body and 14px between the two grids — so the break between what is worn and what is held read as a mistake rather than a division. All six now.
  - **The spell slots are a three by three**, all nine ranks, in the square at the head of the column. A rank the character cannot cast yet is dimmed and marked `--` rather than left out, for the same reason the prepared cells past their limit are drawn: a block that is a different shape at every level is a block you have to read rather than recognise.
    - Each is a **captioned cell** — `Lvl 3` over the number of slots — with its own border, and a tinted band behind the caption running edge to edge. Two lines of bare text in a twenty-six pixel box read as one smudge; the band separates the label from the figure without spending a border or a pixel of gap on it. It was nine bare figures on the argument that a cell's position is its rank, which is true and useless until you already know it. Abbreviated because the column is one gear slot wide and each cell a third of that, twenty-six pixels: "Level 3" does not fit there at a size anybody could read. The tooltip says it in full.
  - **An empty cell shows its number**, centred, and nothing else. It shared the box with a sparkles glyph marking the cell as open, and the two spent thirty-nine pixels arguing: the glyph said what the cell's own dashed border and full brightness already say beside a dimmed one, and it pushed the number into a strip along the bottom too small to read at a glance. The number is context while the box is empty ("the eleventh of seventeen") and noise once there is a picture in it, so it still goes when the cell fills.
  - **The cells past a caster's limit are drawn, dimmed, and refuse drops.** They are not empty cells, they are cells this character does not have YET, and showing them is how the column says what levelling up will buy — and a column that lost cells as it filled would stop being a fixed shape. The drop handler refuses them as well as the CSS: `pointer-events: none` is a cursor hint, not a permission check.

- **Prepared spells are one pooled list**, not one list per class. In 5e preparation is a flag on the *spell*: a Cleric 5 / Wizard 3 prepares a single list against a single combined limit, and drawing two grids with two counts invented a rule the game does not have — the same mistake as making Both Hands lock Main and Off. Builds written under the old shape are flattened on read rather than migrated, since a concatenation fixes for free what a migration would fix with a write on every read of every build.
- **A Sheath slot**, square, beside the hands: the dagger or hand axe you draw without thinking about it. It replaces the second ammunition circle, which was never needed — one ammunition slot says "arrows" and a second said nothing a build could use. Square rather than round, because a sheathed weapon is held and used; the circle belongs to ammunition alone, which is spent by the weapon rather than wielded.

- **Costume mode serves its own view**: the build's picture large, the portrait and token beneath it with labels, and the name. The doll is *absent* rather than hidden — a costume never touches gear or spells, so sixteen slots that could only sit empty would be a promise the mode does not make, and the armour class, the plates and the spell column all describe equipment this view has nothing to do with. The window does **not** resize with it — an earlier version did, and a window that moves and resizes under the cursor costs more than the empty space it recovers. The costume fills that space instead: the two images it writes share a row at the top, and its own square picture takes everything under them. They were stacked around it — one above, one below — which spent the column's height on three rows and forced all three to be small; on one row they cost a single row between them and the square gets the rest. The circles are 140px here against 60px on the doll, since there they are two of sixteen and here they are two of three.
...
- **Two quick-use slots on the doll**, taking a spell *or* a feature, at the ends of the Feet row with the ammunition moved in beside the boots. **Only the ammo slots are round.** All four were for a while, on the grounds that none of them is worn — but Rage and Fire Bolt are things you *use*, exactly like the weapons below, and a square says so. Ammunition is the odd one out: it is not used, it is spent by the weapon that fires it, and it keeps the circle to say that. Two rather than a list on a martial's doll: the pair you reach for without thinking. A caster gets **three**, and gets them as the big slots at the bottom — see the layout swap above. They join the weapons on the handle when that option is on, so the strip carries five things at most.
  - A spell **or** a feature — Rage and Second Wind belong here as much as Fire Bolt does. The question the slot asks is "what do you reach for", and restricting it to spells made two slots a barbarian could never fill. A cantrip is welcome too, unlike in a prepared slot: it is the most reachable thing a caster owns.
  - The stored keys stay `spell1` and `spell2`. Renaming them would strip the slot on every build that already has one, and a key is not a label.
...
- **Applying writes the portrait and the token separately, and reads each back.** They were one `actor.update()` carrying both keys, and when the portrait stopped changing there was no way to tell from outside whether the write had been rejected, the path was empty, or the update had run at all — the canvas token repaints through its own call, so a silent failure still looked half-successful. Two writes and a verified read cost one round trip and make the failure legible; the receipt now says "changed portrait" and "changed token" separately, and a portrait that does not take is logged with what was wanted and what is there.
- **An image slot that changes nothing says so**, marked **no change**. It shows the character's *current* artwork when the build has not chosen one, which made it look chosen — and applying writes only the images a build actually set, so an unset portrait leaves the character's alone. The marker read "unset" first, which named the state rather than the consequence; the consequence is the part anybody needs, and it is exactly the case where one image changed on apply and the other did not. The tooltips say it in full.
...
- **Builds can be kept on the tray handle.** Drag one from the builder's rail onto the handle and it joins a strip above the favourites; **clicking it equips that build**, which is the whole point of it being there — switching kit without opening anything. Right-click takes it off. It confirms first like every other route to applying, and carries the first item picture in the build as its face, since a build has no artwork of its own and a column of identical shirts would defeat the purpose of putting them somewhere glanceable.
  ...
  - The drop is recognised by a flag rather than a payload — `dataTransfer` is protected during dragover, so the handle has to know it is being offered a build before the drop lands. The payload it does carry is deliberately not a Foundry document, so "put this build on the handle" can never create something on the canvas.

- **Builds can be equipped.** The runner icon on a build's tile — and an entry on its right-click menu — puts the build on the character. It asks first, and the confirmation *names what will happen* rather than saying "are you sure": how many items will be equipped, that everything else will be unequipped, how many spells will be prepared, whether the artwork changes. Same reasoning as the cleanup window's preview — nobody can evaluate a question that does not say what it will do.
  - **The tile itself still opens the build.** Applying is a separate target on purpose: this is the only control in the tray that re-equips a character, and clicking a build to look at it must never change the sheet.
  - Three rules, chosen so applying twice is a no-op and applying another build afterwards is a clean swap. Gear in the build is equipped and **every other equippable item is unequipped** — a build is a complete statement of what is worn, not a set of additions, or a light kit applied over a heavy one leaves you in both. Spells in the build are prepared and everything else that counts against a limit is unprepared; cantrips and always-prepared spells are untouched, because `countsPrepared` says they are not part of the daily choice. Portrait and token are written **only where the build set them** — a slot showing the character's own face means "this build does not change it".
...
- **An estimated AC**, on every build tile. Armour, shield and Dexterity, with `system.armor.dex` respected as the cap it is — 0 on heavy, absent on light — and magical bonuses included for free, since dnd5e folds them into `armor.value` during preparation. Labelled **est.** everywhere it appears, and that label is not decoration: it cannot see Unarmoured Defence, a Defence fighting style, or a bonus granted through an active effect, so a bare "AC 18" beside a real attunement count would read as a fact about the character rather than a comparison between builds. Comparing builds is the question it answers.

- **Dropping an item on a filled slot reports what changed** — `AC 16 → 18 (+2) · +5 lb`. This is the honest form of "which is better": no invented score, just the two numbers that actually moved, and silence when nothing did. A torch swap says nothing; a breastplate that costs two points of AC says so before the window is closed.

- **Hovering anything holding an item shows the system's own item card**, gear and prepared spells alike — the same card the tray rows and the character sheet show. Empty slots keep their "drag something here" text, which is the more useful thing to say about an empty slot.

- **Rail entries are tiles carrying the build's own picture**, the way a favourite tile carries its item's — the image behind a dark gradient, with the name and figures over it. The gradient is not decoration: pale text has to stay readable over a picture nobody chose for the purpose, which is exactly what a flat panel fails at. The name and stats take fixed light colours rather than the theme's, since they sit on artwork rather than on the window's own surface and a theme colour there is a coin flip that comes up dark-on-dark in the light theme.
  - Three marks on the tile: **portrait, token and main hand** — how you look, how you look on the map, and what you are holding. It was five gear thumbnails, which said what was *in* the build, and the doll shows that in full the moment it is selected; five item icons at 20px is a row of smudges rather than a summary. The main hand earns a mark of its own because it is what most distinguishes two builds that dress the same. The two images are circles and the weapon is square, which says which of the three is a different kind of thing without spending a label on it.
  - The handle's build icons wear the build's own picture too. They borrowed the first item in the build because a build had no artwork of its own; it does now, so the borrowing is gone — and `buildSummary`'s `preview` field went with it, having lost its last caller.

- **Gear builds.** A **Builds** section above Favorites holds a tile per build; the **+** in its header makes one and opens it. A build is a paper doll — sixteen slots around the character's portrait, laid out head/face/neck, back/chest, arms/hands, rings and hips around a waist, feet, then main hand / both hands / off hand — and you drag items from the tray into them. Built on Blacksmith's tool window (`BlacksmithToolWindowBaseV2`), styling content only and taking its colours from the `--blacksmith-tool-*` variables rather than fighting the shell.
  - **A build is a saved plan and writes nothing to the sheet.** No `system.equipped`, no item updates — filling a slot changes only Squire's own `builds` actor flag. An "equip this build" action is a separate feature with its own questions (what gets unequipped, what about attunement, what about items no longer owned), and the window says so at its foot, because a ring of slots around a portrait otherwise reads as a statement about the character right now.
  - **The character's own portrait and token are captured before any build can touch them.** Applying a build will overwrite `actor.img` and the prototype token's texture, and the moment it does the real artwork exists nowhere — a second apply would then "restore" to whatever the first one set. So it is recorded in a `defaultImages` flag the first time a build window opens against that actor, which is the earliest possible moment and comfortably before anything can overwrite it. The capture writes only when the flag is absent: a capture that ran twice would be worse than none, because the second run would look like a success while recording a costume as the face. `restoreDefaultImages()` is the other half and is ready for the apply step; it deliberately does not clear the flag, since the defaults are the character's identity rather than a one-shot undo.
  - Unset image slots fall back to that captured artwork rather than to whatever the actor currently shows. Once builds can be applied those stop being the same thing, and an unset slot showing the previous build's costume would be the wrong answer to "what does this build change".

  - **Portrait and token slots flank the head.** Round, like the ammo slots, and a click rather than a drop — they hold an image path, so there is nothing on the sheet to drag into them. Clicking opens Foundry's own file picker, started at whatever the slot is currently showing, because the common edit is a variation on the current picture rather than a search from the root; right-click resets. A slot the build has not set falls back to the character's own portrait or prototype token artwork, so the pair always shows a face rather than two empty circles, and stays dashed to say the build changes nothing there — "unset" and "set to exactly what it already is" are different intentions, and only the second should survive being applied. These are groundwork: applying a build will eventually set the actor's portrait and token from them.
  - The window body uses the same 8px padding `.squire-tool-content` gives the cleanup and transfer windows. It was at 4px and read as cramped beside them; a tool window in this suite should not be identifiable by its margins.

  - **Duplicate a build** from its tile's right-click menu, landing directly after the original because a variant belongs beside the thing it came from. The copy carries the same item ids, which is correct — both plans name the same longsword — and nothing is cloned on the sheet, since a build has never owned an item, only pointed at one. Each class's prepared list is copied rather than shared, or editing one build's spells would edit the other's.
  - **The footer totals the gear's weight.** Gear only: prepared spells weigh nothing. Quantity is deliberately ignored — a build slots one arrow to mean "arrows", not to mean the whole stack of sixty, so multiplying out would report a weight nobody is carrying because of this plan. Absent entirely when nothing in the build has a weight, rather than claiming a confident zero.
  - **The build name reads as a heading and edits as a field** — no box or border until you go near it. As a full-width boxed input it was the biggest thing in the window and the least important thing in it, which is a poor trade for a word most people set once.

  - **A filled slot keeps its glyph in the corner.** Once artwork covers a slot the label is gone, and a full doll is otherwise sixteen pictures with no way to tell which hole each is filling. Small and faint, so it reminds rather than competes. The two corners now mean the same thing everywhere in the window: one marks *what kind of thing this is* (the slot's glyph, a spell's level, a stack's count), the other *what it asks of you while in play* (attunement, or concentration).
  - **Prepared spells show their level, and concentration is marked.** Concentration is marked and never counted: preparing six concentration spells is perfectly legal — you simply cannot run two at once — so it is a fact about each spell rather than a limit a build can exceed, and a total would imply a constraint the game does not have.
  - The weapons row got its separation back. It lost it when the doll gained a wrapper for the two-column layout: the body grid and the weapons grid became plain block siblings with nothing between them.

  - **Cantrips are gone from the window.** They are always available, never prepared and never chosen, so the strip was a row of pictures nobody could act on, taking height from the list that matters.
  - **The slot readout appears once**, in the square at the head of the pack column, rather than per class. Spell slots are the character's and not any one class's — a multiclass caster casts from a single pool — so a copy for each would be the same numbers claiming to be different ones.

- **Casters get a prepared-spell column.** One section per class that actually prepares, sized to that class's own limit, with a `3 / 9 prepared` count. dnd5e computes `preparation.max` from each class's formula and knows which spells count via `countsPrepared`, so nothing here re-derives "ability modifier plus level" — a rule with a decade of exceptions the system already owns. A **known** caster (sorcerer, bard, warlock) has no preparation maximum and gets no section rather than an empty one; their list is not a daily choice. A multiclass caster gets one section per preparing class, because the limits are independent and a combined figure would be flatly wrong. The window opens 840px wide for a caster and 520 for everyone else, rather than leaving 300px of nothing.
  - **What the spell column deliberately is not:** a grid of slots by spell level. In 5e you prepare a *list* and then cast any prepared spell with any slot of sufficient level — there is no "this spell lives in that slot". Modelling it that way would look right and invent a rule the game does not have, the same mistake as making Both Hands lock Main and Off. The slot pyramid and the cantrip strip are readouts beside the list, not containers.
  - Spell slots validate where gear slots do not, because here there is something real to check: a non-spell is refused, and so is a cantrip — it is always available and never counts, so letting one occupy a prepared slot would make the count wrong in the one place the count is the point. Prepared lists are padded and truncated to the class's current limit on every read, so levelling up widens the list with no migration and a lost preparation does not keep its overflow alive invisibly.

  - **Filled slots glow by rarity.** `system.rarity` runs common through artifact, and the border plus a soft outer glow carry it — not the artwork, which would fight whatever the item's own picture is doing, and not the caption, which is 0.6em text on a dark gradient. Unrated items stay plain, which is the point: the colour means something because most of what a character owns has none. The key is camel-cased back to `CONFIG.DND5E.itemRarity`'s form first, since the field holds whatever the sheet wrote.
  - **Attunement is counted, and shown per slot.** A lit gem on an attuned item, a hollow warm one on an item that *requires* attunement and has not got it — the second being the case worth seeing, because that item is in the build and would do nothing. The footer totals it as **"2 / 3 attuned"**, drawn from `system.attributes.attunement.max` so a world that changed the allowance is respected, and turns warm when a build spends more than it has. Not an error: planning past your limit is a thing you might be doing on purpose, it should just be impossible to miss. Counted over the build's own slots rather than the whole sheet, because the question is whether *this set of gear* fits inside three.
  - **The three weapon slots are thirds, so they are the biggest thing in the window** — everything above is worn, these are what the character swings, and a doll with its weight at the bottom reads as a character rather than a list. They were briefly on the body's five columns to make them square; `aspect-ratio` on the slot means a third of the width is square at a third of the width, so squareness no longer costs them their size.
  - **Two round ammo slots** flank Feet, directly above the weapons they feed. Round because ammo is the one thing on the doll that is neither worn nor held, and a circle says "different kind of thing" without spending a word on it. Two rather than one per weapon: a quiver is something you carry, not something each hand carries. A round slot's caption sits under it rather than in a bar across the bottom, since the corners of a gradient would fall outside the curve.
  - Every slot is square, stated as an `aspect-ratio` on the slot so the grid rows follow it at any window width. A flat row height was the first answer and it is what made them rectangles — the columns are a fifth of the window and the rows were a fixed 62px, so the two could only have agreed by accident. The weapon row moved onto the body's five-column track with its three slots at columns 1, 3 and 5: equal thirds made those the only slots that could not be square, because a third of the width is not a fifth of it.
  - **A filled slot is drawn like a favourites tile** — the artwork fills it edge to edge with the item's name over the bottom under a gradient, rather than a thumbnail in a box. Same reasoning as there: a flat caption panel becomes a band of colour across every slot, and a text shadow disappears on a pale icon. An empty slot shows the glyph for what belongs in it (a helmet, a ring, a boot, a shield) over its label, because a word alone in a 62px box is a label with nothing to label. Two glyphs are substitutes rather than matches: Font Awesome has no belt, so the waist wears `fa-grip-lines`, and the back gets a backpack, which is at least worn there.
  - **Only items the character owns.** Slots store an item id and resolve synchronously against the actor; a drop from a compendium or another sheet is refused with a warning rather than stored as an id that would never resolve. An item that later leaves the sheet leaves its slot marked missing rather than blank — the difference between "never filled" and "the sword you put here is gone" is most of why anyone opens an old build — and the id stays in the flag, so re-importing the item restores the slot.
  - **Slots enforce exactly where dnd5e has a field to answer the question, and nowhere else.** There is no general equipment-slot concept in the system, but "most slots cannot be checked" is not "none can":
    - **Nothing non-physical, anywhere.** A spell, a feat, a class entry or a background is not an object, and `item.type` says so outright.
    - **`ammo` is a real `consumableTypes` key** that content reliably sets, because ammunition is functionally special in dnd5e — weapon attacks consume it. So the ammo slots only take ammunition; a healing potion is refused.
    - **Both Hands takes a weapon**, because nothing else is ever wielded in two. It still does not lock Main and Off: which weapons are two-handed is a rule the system knows and this module would have to re-derive and be wrong about.
    - **Every other slot takes any physical item**, and the reason is worth stating once because it caught two rules that looked defensible and were not: *a key existing in CONFIG is not the same as content using it.* `ring` is a real `equipmentTypes` key, so the ring slots were validated on it — and then refused a Ring of Fire Resistance, because published content types rings as `trinket` and very little sets `ring` at all. "Weapon or shield" on the hands went the same way when it refused a torch. Both were grounded in a field that existed, and both turned down more real items than the imaginary ones they caught.
    - Every refusal names what the slot wanted. "Nothing happened" is the least useful possible answer to a drag that failed.
...
- **Favourites can be sorted: manual, alphabetical, or by category.** The icon at the right of the Favorites header *is* the current sort, and clicking it opens the three choices with the one you are on shown but not clickable. Category runs in the tray's own panel order — weapons, spells, feats, then the inventory kinds — with names alphabetised inside each block, so a sorted list reads down in the same sequence as the section tabs. An item type Squire does not know about gets a block of its own at the end rather than scattering through the weapons.
  ...
  - In the tile layout each group carries its own grid and the headings sit between them, rather than one grid holding headings and tiles alike. A heading inside the grid was a grid item, so `grid-auto-rows` stretched it to a tile's height — a section title in a 116px slab — and nothing fixes that from inside: `align-self: start` shrinks the element but leaves the row, and the row height is exactly what the tiles need. A grid per group also makes `grid-auto-flow: dense` safe under headings, where one shared grid would have let a small weapon backfill a hole left among the spells.
  - **Sorting never writes to the favourites flag.** Alphabetical and category are lenses over the manual order, so switching away and back returns a hand-arranged list untouched. That is also why the move-up and move-down entries disappear from the right-click menu under a sort: they write positions into the flag, and under a sort those positions are not what is on screen — "Move Up" would rewrite an order you cannot see and look like it had done nothing.

- **A favourite tile can be given more of the grid.** Right-click one and pick 1×1, 2×1, 1×2 or 2×2 from the Tile Size flyout — width first, the way CSS grid reads. It joins the reorder entries already on that menu and appears only in the tile layout, since offering to make a list row two cells tall would be offering nothing. The size you are already on is shown but not clickable, so the menu says where you are instead of leaving you to infer it. The two double-width sizes give their name a larger type size and a third line before it clips.
  - Stored per actor in a `favoriteSpans` map, not as a flag on the item: it is a fact about this panel's layout, not about the longsword, and writing it to the item would put a Squire key on a document that may be shared, moved or exported to say something meaningless anywhere else. 1×1 is stored as absence, so the map holds only the tiles somebody actually resized.
  - The grid's rows are a stated 116px now instead of the tiles carrying `aspect-ratio: 1`. A 2×1 cannot express its shape as a ratio — its width is two columns *plus the gap between them*, and `aspect-ratio` cannot see the gap. The cost is that cells go gently oblong as the tray widens past its 400px floor, which is the same trade the merchant module's shop wall makes and the reason its rows are a stated height too. `grid-auto-flow: dense` backfills the hole a big tile leaves beside a column of small ones.

- **Favourites draw as tiles as well as a list.** A grid of squares, each one the item's own artwork with the name across the bottom, the row controls on a plate top-right and the quantity opposite. Borrowed from the merchant module's shop wall, with one deliberate difference: merchant packs three tile sizes and computes an explicit placement for each, because it sells the leftover cells as advertising and has to know which are free. Nothing here is for sale, so every favourite is an equal square in a plain grid and the browser places them. Three across at the tray's 400px floor, `auto-fill` so a wider tray gets a fourth column rather than three fatter squares.
  - The tiles are the list's markup under a different class, not a second template. Every control on a favourites row is delegated from the panel, so re-shaping rows in CSS keeps rolling, equipping, unfavouriting and drag-to-handle working with nothing to keep in step. The hover-to-roll overlay is `inset: 0` on its container, so on a tile it becomes a full-tile scrim with the die in the middle -- the list's behaviour scaled up rather than a new one.
  - The caption is two lines: the name, bigger and heavier in a warm cream, then kind and weight ("Weapon · 4 lb") with the action badge and quantity at the far end of the same line. A grid rather than a flex column, because line two is two things at opposite ends. Spells say their level instead of a weight, having none and being sorted by the other thing. The detail line is drawn only by the tiles -- a list row has the whole panel width and does not need telling that a longsword is a weapon.
  - The list/tiles switch sits at the right of the Favorites header rather than in the Character Sheet strip: it is a choice *within* this view, and the strip's icons say which view you are in. Two icons rather than one that toggles, because a single button has to show either the state or the action and is read as the wrong one half the time.

### Fixed
- **Hovering a favourite's name gives back the Foundry item card, and the feather now shows it too.** The tile caption was `pointer-events: none` as a whole block so that clicks fell through to the roll overlay, which also meant the name could not be hovered -- and the name is where `applyItemTooltips` hangs the card. The caption stays inert; the name and the editable badges take their own events back. The feather gets the same card in every item panel, which is a straight gain: it is the control that opens the sheet, so the card previews exactly what clicking it will show, and on a tile it is an easier target than two clipped lines of name in a corner. Those feathers carried no tooltip of their own to displace.

### Changed
- **The action chip row had 14px of air under it and 6px over it.** The row's own `margin-bottom` stacked on top of the 8px `.panel-containers` already contributes, which is not visible from either rule on its own — five small glyphs were sitting in a band of empty space. Now 1px under and 3px over, with the totals doing the work.
- **The favourites and quick-add views end on a heading with another heading right under it.** With the search box, tabs and chips all hidden, the Character Sheet strip is just its title — and the next thing down is the Favorites bar, a filled black block. Two headings back to back in two different treatments read as a mistake. They get 8px between them now rather than being made to match: they *are* different kinds of heading, one naming the panel you are in and the other the list below it, so the fix is to stop them touching. Sheet mode never showed this because the search box and tabs sit between the two.

- **Docs caught up with the session's changes.** The README's "fourteen chips over one search box" and the architecture files' descriptions of the Control panel, the player-view panel order and the party tab's containers all still described the arrangement these changes replaced. The settings guide claimed the item-type chips were remembered and the action ones were not, which is now three remembered things (view, tab, favourites layout and sort) and two deliberately forgotten ones. Tray Width's documented floor was still 350.

- **The filter bar sits at its two ends instead of spreading across the row.** Action costs left, availability right, and the hairline between them is gone — the ends now do the dividing it was drawn to do. An even spread was the right answer when four groups of chips competed for one row; with two groups left it read as five chips scattered across half a bar for no reason. The five chips also gained 2px of gap, so they still read as five things once they are sitting together.

- **The Character tab reopens on the view you left it in, and starts on favourites.** `controlMode` remembers 'sheet' or 'favorites'. Quick-add is never written to it: it is something you are in the middle of doing rather than a place you live, and restoring into it would also mean restoring into a mode the GM may since have revoked access to.
- **The mode icons have room to breathe, and the lit heart matches its neighbours.** The cluster's gap went from 4px to 7px when the heart made it three: at 4px a row of three glyphs reads as one shape rather than three targets, and each is a click target barely wider than its own ink. The heart's red `.active` colour is gone -- it now dims and lights exactly like the person and the magnifying glass, which is what a three-way switch should look like.

- **Favorites is a view of its own, behind a heart in the Character Sheet strip.** It was pinned above whichever tab was open, which meant a favourite weapon appeared twice on the Weapons tab -- once in the pinned block and once in the section below it -- and the block ate the top of a column where vertical room is the scarce thing. The heart sits left of the person icon; the view has no tabs, no search box and no filter bar, because there are no sections to move between, no action costs worth filtering a hand-curated shortlist by, and a search box over a list you built yourself is a control looking for a job. The list keeps its own header and its **Clear All Favorites** button, which is the one control it does need.
  - `_compendiumMode` was a boolean and the title bar is now a three-way switch, so it is a `_mode` string over `MODES` -- sheet, favorites, search. The person icon is no longer gated behind `canUseCompendiums`: it used to be half of a two-state pair whose other half was compendiums, and now there is always somewhere to switch back from.
  - Favourites are out of the tab and filter machinery entirely rather than exempted from it case by case. `SHEET_PANELS` is the four the tabs govern; `_applyFilters` returns early outside the sheet, so a favourites row is never stamped rather than stamped-and-cleared, and there is no state to fall out of step.
  - The row-level item-type predicate is gone with them. It existed because Favorites could hold anything while sitting above every tab; every remaining panel is homogeneous (inventory takes equipment/consumable/tool/loot/containers, features takes only feats), so it could no longer fire. `FILTER_REASONS` drops `'type'` and `tray-item.css` drops the matching rule -- four reasons now, not five.
  - `filtered-empty` is explicitly cleared from the favourites container. Panel containers outlive their contents, the class is now only ever *set* inside the sheet loop, and a stale one would beat `.visible` and leave the heart opening onto nothing.

- **The Character tab's fourteen-chip filter bar is five section tabs, five action chips and two toggles.** The bar had two problems and they needed different fixes. Fourteen controls of identical visual weight is too many to leverage; nine of them persisting across sessions is how you end up staring at a half-empty sheet with no idea which dimmed icon did it. `settings.js` had already named that failure -- "logging in a week later to a half-empty sheet, with one dimmed chip as the only clue, is a bad morning" -- and exempted the five action chips from persistence on those grounds, while persisting nine others anyway.
  - **The five item-type chips are now a tab strip: All, Weapons, Spells, Feats, Inventory.** They were a multi-select whose common use was single-select, and all five could be off at once. A tab has no off state, so the item-type dimension can no longer be filtered into nothing, and where you are is a lit label rather than an inference from which of five icons is not dimmed. This loses showing weapons and spells together without inventory; All covers the common case and the intermediate combinations were not worth the failure mode.
  - **Favorites is out of that group and pinned above whatever the tab shows.** It is a flag rather than an item kind, which the old code said in as many words while still listing it as the first of five type chips. On a specific tab favourites narrow to match, so the Weapons tab does not show a Favorites block full of spells.
  - **The four equipped/unequipped/prepared/unprepared chips are two "only" toggles.** Show all, or show only what is usable. This deliberately gives back "what am I carrying but not wearing", which the four-chip split existed to make askable -- a real question, a rare one, and worth trading for two controls that read at a glance. The toggles are global, not per-tab: per-tab state makes the All tab unanswerable, since Weapons saying only-equipped and Inventory saying all leaves All to invent a rule. Which toggle is *shown* follows the tab (Weapons and Inventory share Equipped, Spells owns Prepared, Feats shows neither, All shows both); what they mean never does.
  - **Neither toggle persists, and the tab is the only thing that does.** A global lens set last Tuesday is worse than a per-tab one, not better. The tab persists because a lit, labelled tab explains itself on sight, which is exactly what the chips could not do.
  - The safety property is in `AVAILABILITY`: "only equipped" hides rows that say `unequipped`, rather than rows that fail to say `equipped`. A spell carries no equip state at all, so the first reading leaves it alone and the second would empty your spell list. Same shape as the rule the chips already followed, inverted.
  - The open tab's own panel never collapses when a filter empties it. Favorites still does -- a heading over nothing is noise -- but a character with no weapons who opens the Weapons tab gets "No weapons available" rather than a blank tray under a lit tab.
  - Adding an item from a compendium now switches to the tab that holds it. Adding a longbow with the Spells tab open used to leave you on a sheet with no longbow on it, while the reveal helper polled a hidden panel and gave up in silence.
  - Nine settings deleted (`filterType*`, `filterShow*`), one added (`controlActiveTab`). Shift-click solo now applies only to the action chips, the one group left with siblings to solo against.

- **Minimum tray width is 400px, up from 350.** Groundwork for the section tab strip, which has to fit five labels across it -- below 400 they truncate or fall back to icons, and icons are what the tabs are replacing. The floor is `SQUIRE.TRAY_MIN_WIDTH` rather than a literal in the slider, because raising a range setting's `min` does not touch what is already stored: anyone sitting at 350 or 375 would have kept that width, expressible by no position on the slider they were looking at. It is clamped once at ready so the stored value and the control agree again.
...
- **Adopted the suite-wide documentation standard.** `documents/` is `documentation/`, which is the whole reason none of the shared tooling reached Squire before now -- nothing looks in `documents/`. The three architecture files were already correctly named and moved with `git mv` so history follows; each gained the uniform header and audience line the standard requires, and `architecture-squire.md` no longer describes Notes, the Codex and Quests as Squire's, which they have not been since 13.7.0.
  - The two `TODO.md` files are one. The root file was already a redirect stub carrying no live work, so it is deleted rather than merged; `documentation/TODO.md` is the single list the standard allows.
  - Cross-repository entries are out of Squire's TODO -- five tracking rows and six items, every one of them work owed by Blacksmith rather than by Squire. The standard puts cross-module work in the hub's `TODO-GLOBAL.md`. The extracted text was handed to the author to place there; nothing was dropped.
  - Verified by `node tools/check-docs-structure.mjs`, which reports OK across 8 documents, 7 published, and by `node tools/wiki-sync.mjs build`, which builds all seven pages plus Home and the sidebar with every internal link resolving to a real page.

- **README rewritten as a product page.** It was a feature dump that had drifted from the code: it advertised a **Color Theme** setting whose registration has been commented out in `settings.js` since before this release, a **Maximum Handle Favorites** setting that does not exist, and three "Remember ... Filter" settings replaced by the `filterType*` and `filterShow*` keys, which are stored silently and have no settings-page control. It also pinned Blacksmith at 13.12.2 while `module.json` requires 13.19.1. All four claims are gone, and the file now follows the standard's order: what it is, what it does, requirements, install, where to read more, the suite, licence.
  - It carries the suite's AI-assistance disclosure verbatim between `global:ai-assistance` markers, copied from Blacksmith's `documentation/global/global-ai-assistance.md`. The structure checker compares the two and fails on drift, so it is edited there and nowhere else.
  - Verified by the structure checker's shared-block report, which now finds the disclosure present rather than missing.

### Added
- **Eleven screenshots, and the five guide corrections they caught.** `documentation/assets/` carries current captures: two product screens the README and `home.md` both draw on, and nine the getting-started guide references. Named for the document that owns them, `product-` for the two shared ones, per the assets rule.
  - The guides were written from source, and source told them five things wrong. The panel header reads **Summary**, not Character Summary -- only the setting says Character Summary. The Party tab has a **Party Health** panel that was not documented at all. Compendium mode replaces the filter bar as well as the panel stack, and the results panel is titled **Add from Compendiums**. The **Clear search and keep open on add** tickbox sits under the search box, not under the results. And Cleanup's undo covers the merge alone -- the window itself warns in as many words that there is no undo for the run, which the guide had implied there was.
  - This is the screenshot rule earning its place. Every one of those was invisible in the source and obvious in an image.
  - The seven files in `product/` are now superseded and referenced by nothing; retiring them is a TODO rather than part of this change, because three of them document Quests, the Codex and Notes and may be worth handing to Librarian and Blacksmith rather than deleting.

- **`documentation/home.md`, `known-issues.md`, and two user guides.** Home routes rather than explains; known-issues carries three real, unfixed defects -- player-to-player currency needing write access to both characters (`panel-inventory.js:683`), the die overlay on a container row that opens rather than rolls (`helpers.js:654`), and the handle's all-or-nothing conditions grid. `userguide-getting-started.md` and `userguide-settings.md` are written for a person at the table: how to open, pin and resize the tray, how to point it at a character, what each panel does, and every settings control by its on-screen name.
  - Both guides are drafts until somebody walks them in a running world. They were written from source and from the templates' literal English labels, which tells you what exists but not what a screen calls it or what order things appear in. `documentation/assets/` holds only a `.gitkeep`, and no screenshots are published, because every image in `product/` predates the module split and shows a five-tab strip -- TOKEN, PARTY, NOTES, CODEX, QUESTS -- against the two tabs the tray has today.

- **Re-synced two publisher files after the hub moved.** `.gitattributes` gained `*.webp binary`, and `tools/check-docs-structure.mjs` now scans `testing/` for emoji as well as `documentation/` -- the emoji rule was going unenforced in exactly the place the standard tells people to put a testing document. Both re-copied; all five verified byte-identical to the hub's committed blobs again.
  - The `.webp` rule arrived a commit after Squire committed eleven of them. Checked rather than assumed: every committed blob is byte-identical to the file on disk, because `text=auto` detects the NUL bytes in a WebP and leaves it alone. The explicit rule removes the reliance on that detection.

- **The five publisher files, copied from Blacksmith unchanged**: `.gitattributes` first, then `tools/wiki-sync.mjs`, `tools/check-docs-structure.mjs`, `tools/.gitignore`, and `.github/workflows/sync-wiki.yml`. Squire had no `.gitattributes` and no wiki pipeline at all, only `release.yml`. Nothing in any of the five is edited per module; identity and branch are derived at run time from `module.json` and the repository.
  - Verified by comparing each staged blob against the hub's committed blob rather than the working-tree file. `core.autocrlf` is true here, so a working-tree diff would have gone on looking clean until the next checkout and then reported every line changed, which reads exactly like a local edit that never happened. All five match.

- **`testing/TESTING.md` discharged its shipped section and now declares its own state.** The 13.6.1 quest-removal section was thirteen ticked boxes for a release two versions back, which the standard's testing rule and the file's own preamble both say to delete rather than leave ticked -- a document full of ticks is indistinguishable from one nobody has run. Gone, along with the stray code fences three of its items had grown. The two remaining sections, the codex extraction and the favourites sheet sync, are 114 open items between them and neither has been walked; the file says so at the top now, which testing rule 1 requires and it was not doing.
  - Also fixed a missing line break that had run the "Codex removed" heading onto the end of the previous section's last item, so it was not rendering as a heading at all.

### Fixed
- **Character sheet print never opened in the Foundry desktop client** (`utility-print-character.js`). It called `window.open('', '_blank')` and, when that returned null, reported "Pop-up blocked. Please allow pop-ups for this site." -- sending people to a browser setting that could not have helped. The desktop client is Electron, where a renderer may only open a window if the main process registered a `setWindowOpenHandler`, and Foundry's does not. No pop-up blocker is involved and there is nothing a user can allow.
  - It was fragile in the browser too, for a separate reason: the `window.open` ran after `renderTemplate()` and several other awaits, so it was spending the click's transient user activation before asking for the window. It worked because Chrome allows roughly five seconds, not because it was correct.
  - The sheet now opens in a Foundry window -- `scripts/window-print-character.js`, on Blacksmith's tool-window base like Cleanup and Transfer, with `styles/window-print-character.css`. Same window in both clients, resizable, position remembered, one per actor.
  - It holds the sheet in an **iframe** rather than inlining it. The sheet is a complete HTML document with its own `head`, `body` rules, `@page` and `@media print` blocks; inlining would drop all of that into the same cascade as Foundry's and Squire's. The iframe also makes the Print button correct, because printing an iframe prints its document rather than the application around it.
  - **Print or Save as PDF** is a button in the window's footer, not something that happens on the way in. The sheet is worth reading; a print dialog opening over it answers a question nobody asked. It focuses the frame before printing -- an unfocused frame prints the parent document in some engines, which would print Foundry instead of the sheet.
  - Removed with the popup: `PRINT_CONFIG.ERROR_MESSAGES.POPUP_BLOCKED` and `IMAGE_LOAD_TIMEOUT`, the image-wait race the popup needed, and -- already dead before this change -- an uncalled `getBlacksmith()` helper and the unused `MODULE`, `SQUIRE` and `moduleDelay` imports.
  - Verified statically: `python testing/preflight.py` passes across 34 files and `python testing/css-check.py` across 22 stylesheets. The window itself is unproven in either client and is logged in `testing/TESTING.md`.
  - Note for another repo: `coffee-pub-scribe/scripts/scribe.js:460` carries the identical `window.open('', '_blank')` pattern and will be broken in the desktop client the same way.

- **`testing/TESTING.md` discharged its shipped section and now declares its own state.** The 13.6.1 quest-removal section was thirteen ticked boxes for a release two versions back, which the standard's testing rule and the file's own preamble both say to delete rather than leave ticked -- a document full of ticks is indistinguishable from one nobody has run. Gone, along with the stray code fences three of its items had grown. The two remaining sections, the codex extraction and the favourites sheet sync, are 114 open items between them and neither has been walked; the file says so at the top now, which testing rule 1 requires and it was not doing.
  - Also fixed a missing line break that had run the "Codex removed" heading onto the end of the previous section's last item, so it was not rendering as a heading at all.

### Fixed
- **Character sheet print never worked in the Foundry desktop client** (`utility-print-character.js`). It called `window.open('', '_blank')` and reported "Pop-up blocked. Please allow pop-ups for this site." when that returned null -- a message that sent people to a browser setting that could not fix it. The desktop client is Electron, where a renderer may only open a window if the main process registered a `setWindowOpenHandler`, and Foundry's does not. There is no pop-up blocker involved and nothing a user can allow.
  - It was fragile in the browser too, for a second reason: the `window.open` ran after `renderTemplate()` and several other awaits, so it was spending the click's transient user activation before asking for the window. It worked because Chrome allows roughly five seconds, not because it was correct.
  - And it never actually printed. The old path opened a window, wrote the HTML, waited for images, and stopped -- printing was left to whoever thought to press Ctrl+P.
  - Now an off-screen iframe, written the same way and printed with `contentWindow.print()`. An iframe is same-origin, can be created at any time, and needs no activation, so both clients take the same path. It is positioned off-screen rather than hidden because a `display: none` frame has no layout, and the sheet is `max-width: 900px` with a `100vh` cover page -- both resolve against the frame's viewport before `@media print` takes over. The frame is removed on `afterprint`, never earlier, because tearing it down while the dialog is open cancels the job, with a 60-second backstop for engines that do not fire the event.
  - `PRINT_CONFIG.ERROR_MESSAGES.POPUP_BLOCKED` is deleted rather than reworded; nothing else referenced it.
  - Verified statically only: `python testing/preflight.py` passes across 33 files. The print itself is unproven in either client and is logged in `testing/TESTING.md`.
  - Note for another repo: `coffee-pub-scribe/scripts/scribe.js:460` carries the identical `window.open('', '_blank')` pattern and will be broken in the desktop client the same way.

- **One emoji removed from the CHANGELOG** (`13.3.x`, the Blacksmith API notes entry). It sat inside a quotation of a Blacksmith heading, so the words are unchanged and only the glyph is gone. The standard's no-emoji rule is absolute and covers the whole repository, published or not, and the structure checker enforces it there.
...
- **Containers were being filtered out of the Inventory panel entirely, and had been for as long as this world has run a current dnd5e.** The item-type filter in `_getItems()` read `backpack` — dnd5e's *old* name for the type — and never gained `container`, the current one. So no bag ever reached the panel: the Containers category has never rendered, and bag view put everything under General because it couldn't find a single container to group by. Both names are accepted now, since an old world can still hold the old type.
  ...
  - Two more places carried the same stale list: the `updateItem` hook that decides which panels to re-render (so editing a bag refreshed nothing) and the item-type icon map.
- **The Inventory panel has a bag view.** A new toggle at the head of its title bar — ahead of the category filters and fenced off from them with the same hairline the control panel uses, because filters say which rows to show and this says how to arrange them — switches between the flat list and a grouping by container: **General** first, then one section per bag, each with the same categories inside it. Rows are untouched, sack icon and all, so the two views are one list read two ways rather than two lists. The choice is per-user.
  - **A container appears only as its own heading, never also as a row inside General.** It is the section; listing it twice would be one object claiming to be in two places. Its row actions stay reachable in list view, which is the view that treats a bag as a thing you carry rather than as a place things are.
  - **Deliberately flat**: a bag inside a bag gets its own top-level section rather than nesting. Nesting is a tree, and a tree in a 400px column is something you navigate instead of something you read — and each row's sack icon already answers "which bag is this actually in".
  - An item whose `system.container` points at an id the actor no longer has falls into General. That's a broken back-reference — the container was deleted out from under it — and the one thing it must not do is vanish.
  - **The bag's sack icon moved from every row to the section heading.** Same class and same data attribute the rows use, so the delegated container listener picks it up with no second handler — one affordance per bag instead of one per row, and the rows stop answering a question the heading above them just answered. The per-row sack stays in list view, which is the view where a row has to say which bag it's in.
  - The heading lost its left rule. It was drawn to make a section look like a container and it did, but nothing else in the tray marks a heading that way, so the one heading wearing it read as a different kind of object rather than as a louder heading.
  - Empty bags keep their section. "This bag is empty" is an answer, and a section that disappears when you take the last thing out of it looks like the bag went with it.
  - **This was a data change rather than a second copy of the list**, because the five near-identical category blocks in `panel-inventory.hbs` became one inline partial and a loop first. Sorting moved into the grouping with them: bag view slices the same items along a different axis, so it has to happen after the slice or each bag's contents come out in collection order.
  - `_updateHeadersVisibility` now scopes its row lookup to the header's own group. Bag view repeats every category once per container, so a panel-wide query would keep an empty "Tools" heading alive in one bag because a *different* bag has tools in it. A bag whose every row is filtered away hides its own heading too.
